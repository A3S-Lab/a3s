...
the root submodule is advanced to Vec `2d73ef96292174316f917fbaaa765a9ca14ff181`
...
plus concurrent-reader/mixed-workload performance artifact gate. The revision-bound
CI run `33650775524` passed all hosted quality,
...
- `crates/vec` is the `A3S-Lab/Vec` git submodule at `2d73ef9`. Format-4
...
performance matrix. The all-feature suite currently passes 260 unit/integration
tests plus four doctests (258 unit/integration tests on both the default and
...
| --- | --- | --- |
| P0 correctness | Real index/recovery behaviour, monotonic revisions, atomic manifest publication, read-only lifecycle, and bounded deserialization | **Engine gate closed** at Vec implementation pin `2d73ef9` (Code's shadow adapter remains separately pinned at `019fdb9`): format-4 snapshots/WAL, all 18 injected publication boundaries, bounded recovery fuzzing, lock ownership, and read-only lifecycle are executable gates |
| P1 contract | Schema WAL replay, typed dimension/type errors, native codec semantics, wired configuration, private kernel boundary, and promised integration tests | **Closed for the advertised engine surface** at root pin `2d73ef9`: generated vector/FTS/filter and Binary32/Binary64 Hamming oracles, advanced FTS, concurrency, private-kernel compile failures, typed unsupported paths, IVF SOAR/cache contracts, public `Send + Sync` contracts, metric-aware Vamana/DiskANN contracts, dense/binary/FTS query-builder execution, include-doc-id persistence checks, and the complete feature matrix plus concurrent-reader/mixed-workload tail-latency gates are tested |
| Strict quality | `cargo fmt --check` and `cargo clippy --all-targets -- -D warnings` for Vec | **Passed locally and in [Vec main CI run `33650775524`](https://github.com/A3S-Lab/Vec/actions/runs/33650775524)** for `2d73ef9`, including feature-matrix, concurrent-reader, and mixed-workload smoke on Linux x86-64/ARM64, Windows x86-64, and macOS ARM64/Intel, MSRV, recovery fuzz, and versioned release-candidate packaging; the dependency audit is additionally green locally. The exact-revision macOS 12 Intel qualification workflow remains available |
