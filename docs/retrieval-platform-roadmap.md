...
the root submodule is advanced to Vec `c2344dda55cc36e4c3e3d3fbab3e3512a12b18e8`
...
implementation's revision-bound CI run `33679131820` passed all hosted quality,
...
- `crates/vec` is the `A3S-Lab/Vec` git submodule at `c2344dd` (the lifecycle
...
| P1 contract | Schema WAL replay, typed dimension/type errors, native codec semantics, wired configuration, private kernel boundary, and promised integration tests | **Closed for the advertised engine surface** at root pin `c2344dd` (implementation `dbd4a75`): generated vector/FTS/filter and Binary32/Binary64 Hamming oracles, advanced FTS, concurrency, private-kernel compile failures, typed unsupported paths, IVF SOAR/cache contracts, public `Send + Sync` contracts, metric-aware Vamana/DiskANN contracts, dense/binary/FTS query-builder execution, include-doc-id persistence checks, schema-only WAL compaction, the complete feature matrix plus concurrent-reader/mixed-workload/scale tail-latency gates, and the 16-row lifecycle/resource/maintenance matrix are tested |
| Strict quality | `cargo fmt --check` and `cargo clippy --all-targets -- -D warnings` for Vec | **Passed locally and in [Vec main CI run `33679131820`](https://github.com/A3S-Lab/Vec/actions/runs/33679131820)** for `c2344dd`; the run includes feature-matrix, concurrent-reader, mixed-workload, scale, and lifecycle smoke on Linux x86-64/ARM64, Windows x86-64, and macOS ARM64/Intel, MSRV, recovery fuzz, and versioned release-candidate packaging; the dependency audit is additionally green locally. The exact-revision macOS 12 Intel qualification workflow remains available |
