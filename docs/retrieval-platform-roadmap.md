...
| Strict quality | `cargo fmt --check` and `cargo clippy --all-targets -- -D warnings` for Vec | **Passed locally and in [Vec main CI run `33592919084`](https://github.com/A3S-Lab/Vec/actions/runs/33592919084)** for `1a2d15f`; the run passed the feature-matrix smoke, all hosted platforms, MSRV, recovery fuzz, and release-candidate package gates, while the exact-revision macOS 12 Intel qualification workflow remains available |
