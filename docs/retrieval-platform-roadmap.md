...
the root submodule is advanced to Vec `1be075a2026ace8ae666e4670acb01f2949e7e30`
...
CI run `33648352510` passed all hosted quality,
...
- `crates/vec` is the `A3S-Lab/Vec` git submodule at `1be075a`. Format-4
...
| --- | --- | --- |
| P0 correctness | Real index/recovery behaviour, monotonic revisions, atomic manifest publication, read-only lifecycle, and bounded deserialization | **Engine gate closed** at Vec implementation pin `1be075a` (Code's shadow adapter remains separately pinned at `019fdb9`): format-4 snapshots/WAL, all 18 injected publication boundaries, bounded recovery fuzzing, lock ownership, and read-only lifecycle are executable gates |
| P1 contract | Schema WAL replay, typed dimension/type errors, native codec semantics, wired configuration, private kernel boundary, and promised integration tests | **Closed for the advertised engine surface** at root pin `1be075a`: generated vector/FTS/filter and Binary32/Binary64 Hamming oracles, advanced FTS, concurrency, private-kernel compile failures, typed unsupported paths, IVF SOAR/cache contracts, public `Send + Sync` contracts, metric-aware Vamana/DiskANN contracts, dense/binary/FTS query-builder execution, include-doc-id persistence checks, and the complete feature matrix plus concurrent-reader/mixed-workload tail-latency gates are tested |
| Strict quality | `cargo fmt --check` and `cargo clippy --all-targets -- -D warnings` for Vec | **Passed locally and in [Vec main CI run `33648352510`](https://github.com/A3S-Lab/Vec/actions/runs/33648352510)** for `1be075a`, including feature-matrix, concurrent-reader, and mixed-workload smoke, hosted platforms, MSRV, recovery fuzz, and versioned release-candidate packaging; the dependency audit is additionally green locally. The exact-revision macOS 12 Intel qualification workflow remains available |
