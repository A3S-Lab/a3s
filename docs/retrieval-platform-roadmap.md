...
the root submodule is advanced to Vec `9ed701ae72e45f7b8f7be9c7db943ed4b64f93f4`
...
latest revision's revision-bound CI run `33686399240` passed all hosted quality,
...
- `crates/vec` is the `A3S-Lab/Vec` git submodule at `9ed701a` (CI hardening over
...
| P1 contract | Schema WAL replay, typed dimension/type errors, native codec semantics, wired configuration, private kernel boundary, and promised integration tests | **Closed for the advertised engine surface** at root pin `9ed701a` (implementation `dbd4a75`): generated vector/FTS/filter and Binary32/Binary64 Hamming oracles, advanced FTS, concurrency, private-kernel compile failures, typed unsupported paths, IVF SOAR/cache contracts, public `Send + Sync` contracts, metric-aware Vamana/DiskANN contracts, dense/binary/FTS query-builder execution, include-doc-id persistence checks, schema-only WAL compaction, the complete feature matrix plus concurrent-reader/mixed-workload/scale tail-latency gates, and the 16-row lifecycle/resource/maintenance matrix are tested |
| Strict quality | `cargo fmt --check` and `cargo clippy --all-targets -- -D warnings` for Vec | **Passed locally and in [Vec main CI run `33686399240`](https://github.com/A3S-Lab/Vec/actions/runs/33686399240)** for `9ed701a`; the run includes feature-matrix, concurrent-reader, mixed-workload, scale, and lifecycle smoke on Linux x86-64/ARM64, Windows x86-64, and macOS ARM64/Intel, MSRV, recovery fuzz, zvec companion syntax, and versioned release-candidate packaging; the dependency audit is additionally green locally. The exact-revision macOS 12 Intel qualification workflow remains available |
