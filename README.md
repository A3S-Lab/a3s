...
Cloud also persists project-scoped canonical Form drafts and immutable
owner-compiled releases plus exact Goal/Plan-bound WorkflowRuns through A3S
ORM/PostgreSQL. The minimal run lifecycle executes Workflow-local `input`,
`transform`, `branch`, and `output` steps with A3S Flow and exposes the same
CQRS authority through REST `1.14.0`, the maintained TypeScript client, CLI,
and Management MCP. Protected Form submissions, HumanTask product surfaces,
human/service/finite-task dispatch, typed capability steps, compensation, and
production recovery remain later gates. Component roadmaps and the
...
| Cloud | R0–E0 is the verified cumulative baseline; the compatibility manifest additionally pins native Form compilation/evaluation, Form/Flow interaction contracts, the tenant-isolated Form draft/immutable-release lifecycle, and the minimal Goal/Plan-bound WorkflowRun lifecycle across REST `1.14.0`, TypeScript client, CLI, and Management MCP. The run slice executes Workflow-local `input`, `transform`, `branch`, and `output` steps through A3S Flow. The complete W0 product remains unavailable: protected submissions, HumanTask, human/service/finite-task dispatch, typed capability steps, compensation, expanded recovery evidence, and W0.4–W0.5 remain open |
