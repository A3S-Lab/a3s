...
| Local agent work | Code owns session, workspace, retrieval, context, memory, and provider qualification, including typed provider/model generation capacity admitted through its shared scheduler. Provider and public-network behavior is qualified separately from the local core. | [Code](crates/code/) · [retrieval roadmap](docs/retrieval-platform-roadmap.md) |
