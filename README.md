...
state/operation v2, lifecycle intent v3, lifecycle diagnostic v1, and CLI plugin
operation record v3. Superseded preview records are rejected with cleanup and
...
completes. Code delegates to this shared Use factory, but injects no production
Runtime selection or Gateway adapter by default, and managed Tools are not yet
projected into TUI/Web AgentSession dynamic tools. Release-backed Tool Tasks,
Tool Services, and HTTP MCP therefore remain unavailable by default rather than
falling back to a package-local launcher.
...
| Use | `main` pins the single current preview baseline: manifest/catalog/receipt v3, plan/host/manager tools v4, lifecycle intent v3, lifecycle diagnostic v1, CLI plugin operation record v3, exact SemVer dependency locks, the sole revision-addressed `state/use/registries.acl` source document with identity-isolated TUF/cache state, install-time Registry selection and provenance-pinned upgrade/uninstall, in-process reviewed Grants and graph apply, shared dependency ownership, canonical `user/current` authority across CLI/TUI/Web plans and Use lifecycle journals, restart-safe hot-plug across Tool, MCP, OKF, A3S Flow, Skill, and UI, Code-owned bounded durable UI state with exact published-generation leases and retained-surface cleanup, Code Web failed-N+1 browser readiness with authority-free candidate documents, fail-closed rollback, terminal stale-plan rejection, fresh-plan retry, and a single successful cutover, secret-free latest/previous lifecycle checkpoint evidence with applying/rolling-back exclusion and same-operation phase identity by intent digest, typed Runtime Service endpoint consumption with Gateway drain → Runtime stop → route removal → Runtime removal, receipt-recoverable exact-generation Runtime Task dispatch with provider/capability/enforcement evidence and drain-safe cleanup, explicit standalone Flow preflight with exact replay, a scope-aware local SQLite/FTS5 OKF Knowledge carrier with cited TUI/Web search and exact published-generation query leases, receipt-accounted scope quota, bounded generations/tombstones, physical cleanup, usage diagnostics, scope-bound integrity audit, derived-index repair, versioned backup/offline verification, CI-enforced real-process Code watcher, TUI first-use, and generic Web lifecycle coverage on Linux/macOS/Windows through a configured replaceable Registry source, and a Windows x86_64 signed Registry/graph/Grant/Flow/OKF CLI lifecycle plus killed-process cutover-recovery gate. Broader operational telemetry, managed Knowledge rollback, coordinated restore and authority recovery, backup rotation, distributed Knowledge placement, reviewed Activity backend bindings, equivalent browser/native readiness outside Code Web, native UI hosting, production Runtime provider selection and Gateway injection, managed Tool projection into TUI/Web AgentSession dynamic tools, Tool Service/HTTP MCP composition, distributed Flow recovery/retention, production Registry operations, broader Windows six-surface/failure-injection parity, and the complete six-surface cross-platform real-process matrix remain release gates. Use is not a released product. |
