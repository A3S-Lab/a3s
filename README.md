...
`DetailPanel`, `Timeline`, and `SectionHeader`; completed tool output and task
summaries use `ConnectorBlock`; file-edit diffs use `DiffView`; `/effort` uses
`LevelSlider` and `ShimmerText`; SPF and `/ide` chrome uses `PanelFrame`; SPF
and `/ide` file metadata use `Breadcrumb`; and the main footer uses
`SessionStatus` plus `ModeLine`. Menu bounds, selection windows, connector rows,
diff wrapping, framed panels, breadcrumbs, shimmer, detail rows, and footer width
