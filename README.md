...
`1.21.0`, the maintained TypeScript client, CLI, and Management MCP expose
protected task reads and versioned claim/release through one Workflow state
machine and the shared Identity Resource Grant evaluator. Public Form
submission, expiry/cancellation coordination, human/service/finite-task
dispatch, typed capability steps, compensation, and production recovery remain
later gates. Component roadmaps and the
...
| Cloud | R0–E0 is the verified cumulative baseline; the compatibility manifest additionally pins native Form compilation/evaluation, Form/Flow interaction contracts, the tenant-isolated Form lifecycle, Goal/Plan-bound WorkflowRuns, and the internal HumanTask decision loop. REST `1.21.0`, TypeScript client, CLI, and Management MCP expose protected task reads plus versioned claim/release through the existing Workflow and Identity authorities. The complete W0 product remains unavailable: public submission, expiry/cancellation coordination, human/service/finite-task dispatch, typed capability steps, compensation, expanded recovery evidence, and W0.4–W0.5 remain open |
