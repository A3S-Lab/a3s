...
The proposed cross-product composition of Cloud, Flow, Boot, ORM, and Form is
defined in the [Workflow platform architecture](compat/workflow-platform-architecture.md)
and its ordered [development plan](compat/workflow-platform-development-plan.md).
Component roadmaps and the compatibility lock remain the sources of truth for
delivered behavior.
...
watcher boundary on Linux, macOS, and Windows; Unix runners additionally gate
TUI first-use and generic Web install, restart, upgrade, and uninstall without
initializing Science.
