...
///
/// Holds the `AuthMiddleware` registry and per-channel rate-limiting
/// counters for auth failures.
#[derive(Clone)]
...
impl AuthLayer {
    ...
    pub async fn verify_request(
        ...
        if self.is_rate_limited(channel, now).await {
            ...
            tracing::warn!(channel = channel, "Rate limited: {}", reason);
            return Err(reason);
        }
    ...
    async fn record_failure(&self, channel: &str, now: i64, reason: &str) {
        ...
        tracing::warn!(channel = channel, "Auth failure: {}", reason);
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_auth_layer_verify_rejected() {
        let mut mw = AuthMiddleware::new();
        mw.register(Box::new(SlackAuth::new("secret")));
        let layer = AuthLayer::new(mw);
