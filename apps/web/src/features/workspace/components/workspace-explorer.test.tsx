...
describe('WorkspaceExplorer context menu operations', () => {
  ...
  it('exposes and executes the Chinese rename shortcut from the tree', () => {
    render(<WorkspaceExplorer actions={{} as WorkspaceActions} onOpenSearch={vi.fn()} />);
    const row = screen.getByRole('treeitem', { name: 'app.ts' });

    fireEvent.contextMenu(row, { clientX: 48, clientY: 72 });
    expect(screen.getByRole('menuitem', { name: '重命名' })).toHaveAttribute('aria-keyshortcuts', 'F2');
    fireEvent.keyDown(screen.getByRole('menu', { name: 'app.ts 操作' }), { key: 'Escape' });
    fireEvent.keyDown(row, { key: 'F2' });

    expect(screen.getByRole('textbox', { name: '文件或文件夹名称' })).toHaveValue('app.ts');
  });
});
