...
describe('IntegrationsSettingsView', () => {
  ...
  it('keeps configured secrets masked and saves one coherent integrations draft', async () => {
    ...
    expect(screen.queryByRole('heading', { name: '连接器' })).not.toBeInTheDocument();
  ...
  it('keeps an existing connector-named entry editable as a regular MCP server', () => {
    ...
    expect(screen.getByLabelText('oomol-connector 服务地址')).toHaveValue('https://connector.example.com/mcp');
    expect(screen.getByLabelText(/oomol-connector HTTP Header值/)).toHaveValue('enabled');
  });
});
