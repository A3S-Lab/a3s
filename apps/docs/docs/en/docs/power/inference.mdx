...
Power 0.5.1 deepens the Colibri-inspired parts of that runtime without creating
...
derived plans remain explicit operator values: Power does not log, persist,
export through placement telemetry, or bind them into execution receipts. This
keeps the Colibri-inspired hierarchy inside the same TEE and integrity plane.

A3S OCR uses this path for its Rust PP-OCRv6 and Unlimited-OCR implementations,
but Power does not embed either model. OCR can opt into the same typed residency
budget during lazy model-session loading; Parser only forwards host policy and
does not implement another hardware planner. The embedded dependency closure
excludes the Power server, HTTP clients, ONNX Runtime, and browser automation.
See the
