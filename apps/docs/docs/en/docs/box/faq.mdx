...
- The full pod sandbox + container lifecycle (`RunPodSandbox`/`runp` → `CreateContainer`/`create` → `StartContainer`/`start` → `ps` → stop → remove → `stopp` → `rmp`). Each pod sandbox maps to one Box microVM.
