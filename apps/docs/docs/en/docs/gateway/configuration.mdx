...
| `management` | Compatibility block for the machine-only Node API listener |
...
### Revisions and rollout boundary
...
<Callout type="warn">
Automatic gradual rollout is unavailable in Gateway. The legacy `rollout {}` syntax is parsed only so validation can return an explicit unsupported-feature error. Use static `traffic_percent` weights; A3S Cloud owns production rollout orchestration. Revision selection is deterministic weighted round-robin (not random).
</Callout>
...
See [Middleware](./middleware/index) for full behavior of every type.
...
The historical `management` block is retained as the compatibility name for an optional, dedicated **Node API** listener (disabled by default and bound to `127.0.0.1:9090`). It never intercepts traffic entrypoints and does not provide an operator dashboard, configuration browser, route/service inventory, audit-event feed, raw ACL validation, or raw reload API. Human-facing operations belong to A3S Cloud.

The bounded machine contract contains `GET /health`, `GET /metrics`, `GET /version`, `POST /snapshots/apply`, and `GET /snapshots/status`, all under `path_prefix`. Requests are guarded in order by path-prefix matching, IP allowlist, and bearer token, with optional TLS/mTLS.
...
<Callout type="warn">
If `auth_token_env` names a variable that is unset at startup, the Node API listener fails to start. Set `auth_token_env = ""` to disable bearer auth, or `allowed_ips = []` to disable IP filtering — these are distinct from omitting the keys. Keep the listener private; A3S Cloud is its intended control-plane client.
</Callout>
