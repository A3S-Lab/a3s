...
- **Middleware** — `rate-limit`, `jwt`, `api-key`, `cors`, `circuit-breaker`,
  and more, attached via `middlewares = [...]`: see [Middleware](./middleware/index).
...
- **Observability & Node API** — Prometheus metrics and the bounded machine-only
  control-plane listener: see [Observability](./observability).
