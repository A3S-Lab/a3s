---
title: A3S Gateway
description: A3S Gateway is the AI Native Traffic Layer for protocol routing, streaming, and local policy enforcement.
---


# A3S Gateway

A3S Gateway is the **AI Native Traffic Layer**. It is built on [hyper](https://hyper.rs/) and [rustls](https://github.com/rustls/rustls), ships as a single statically-linked binary, and is configured exclusively in ACL (Agent Configuration Language). Configuration files must use the `.acl` extension — HCL, TOML, YAML, and JSON config files are rejected at load.

[Performance evidence](https://a3s-lab.github.io/Gateway/#performance) · [Traditional-gateway scope comparison](https://a3s-lab.github.io/Gateway/#comparison) · [Animated ACL walkthrough](https://a3s-lab.github.io/Gateway/#config)
...
- [Middleware](./middleware/index) — the built-in request/response pipeline
- [Services](./services) — load balancing, health checks, sticky sessions, failover, and AI-native scaling
- [Observability](./observability) — Prometheus metrics, access logs, tracing, and the machine-only Node API
...
<Callout type="info">
**Automatic gradual rollout is unavailable.** The retained `rollout { }` syntax returns an explicit validation error. Use static [revision splitting](./services) to shift traffic manually today; A3S Cloud owns production rollout orchestration.
</Callout>
...
- Automatic gradual rollout — unavailable in Gateway; use static revision weights while A3S Cloud owns production rollout orchestration
...
**Middleware (14 HTTP types + TCP filter)** — see [Middleware](./middleware/index)
...
- Machine-only Node API — bounded health, metrics, version, and managed-snapshot endpoints; no operator dashboard or configuration browser
...
- MSRV Rust 1.88; see the [latest release](https://github.com/A3S-Lab/Gateway/releases/latest) for published artifacts and enabled features
