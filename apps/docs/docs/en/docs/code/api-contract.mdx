...
The current SDK surface has no dedicated ephemeral-question helper. Use
explicit history when the call must not mutate the session transcript:
...
Live add/remove operations target a private manager owned by this session.
Agent-global and host-supplied managers are inherited read-only capability
sources, so a session cannot mutate a sibling or global MCP configuration.
