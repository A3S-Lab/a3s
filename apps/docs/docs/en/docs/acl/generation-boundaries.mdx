...
| Document root shape | Root assignments remain assignments for every value kind; one-attribute blocks with distinct block and attribute names retain braces. |
