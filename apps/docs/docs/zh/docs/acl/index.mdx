---
title: A3S ACL
description: A3S 组件使用的类型化 Agent Configuration Language。
---

# A3S ACL

`a3s-acl` 是一个轻量的类型化配置语言，用于 agent 和组件配置。它提供
Rust crate 与 Node.js/TypeScript SDK，可以把 ACL 文本解析成 AST，也可以从结构化
值生成 ACL 文本。

仓库中的 Rust crate 与 Node package source 都是 `0.3.0`。Rust crate 已发布到
crates.io。在当前 snapshot 中，`@a3s-lab/acl` `0.3.0` 的 npm publication
仍由 [`A3S-Lab/ACL#13`](https://github.com/A3S-Lab/ACL/issues/13) 跟踪；
在该 release 成功前，registry consumer 不能假定 npm 已提供仓库中的 Node APIs。
crate repository metadata 指向
[`A3S-Lab/ACL`](https://github.com/A3S-Lab/ACL)。

## 能力地图

| 领域 | 当前能力 |
| --- | --- |
| 语法 | ACL attributes、带 label 的 blocks、nested blocks、lists、objects、strings、numbers、booleans、null、function calls、comments，以及 `=` / `:` assignments。 |
| Lexer/parser | Bounded parsing 和 multi-diagnostic collection、stable diagnostic codes、UTF-8 byte spans、comment skipping、string escapes、negative numbers 和 scientific notation。 |
| Admission | Closed-by-default declarative schemas、recursive value rules、stable logical paths、redacted diagnostics，以及 bounded Rust/Node validation reports。 |
| 函数 | 支持把 `env(...)`、`concat(...)` 和自定义函数名表示为 first-class call values。 |
| AST | `Document`、`Block`、typed values、builders、value constructors 和 `Value` accessors。 |
| 生成 | Native ACL output、block-header labels、sorted attributes 和 objects、type-stable strings，以及 map generation。 |
| 完整性 | Rust/Node byte-identical canonical UTF-8、带 algorithm prefix 的 SHA-256 digests，以及对 declared unordered block occurrences 的 recursive schema-aware normalization。 |
| SDK | Rust crate `a3s-acl`；Node package `@a3s-lab/acl`，带 TypeScript definitions。 |

## 从这里开始

- [Syntax](/docs/acl/syntax) 说明 blocks、attributes、values、calls、
  comments 和 parser boundaries。
- [Rust API](/docs/acl/rust-api) 覆盖 bounded parsing、schema admission、
  multi-diagnostic collection、schema-aware canonical digests、generation、
  builders 和 AST value helpers。
- [Node SDK](/docs/acl/node-sdk) 覆盖 JavaScript/TypeScript parser、
  schema validator、native ACL generator、schema-aware canonical APIs 和
  builders。
- [Generation And Boundaries](/docs/acl/generation-boundaries) 说明 stable
  output、canonical bytes、secret handling 和 host-component ownership。

## 示例

```text
default_model = "provider/model-id"

providers "provider" {
  api_key = env("PROVIDER_API_KEY")
  base_url = env("PROVIDER_BASE_URL")

  models "model-id" {
    tool_call = true
  }
}
```

## 在 A3S 中的位置

ACL 是 A3S Code session、Sentry policy 文件，以及其它需要可审阅、类型化产品配置的
组件所使用的配置格式。真实私有值应留在环境变量或宿主密钥系统中。

## 边界

ACL 本身不是策略引擎。组件决定如何解释值、允许哪些函数，以及如何解析敏感值。
