...
- Bundled daemon 会 audit incomplete-evidence escalation，但不提供 durable
  ...
  和 dispatch。
- `PolicyEnvelope::verify` 证明 canonical policy bytes 与精确 desired-state
  binding。当前 daemon 不会应用 envelope、在 restart 后恢复它，也不会根据
  applied digest gate workload readiness。
