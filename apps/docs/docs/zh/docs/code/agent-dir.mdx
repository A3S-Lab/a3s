...
| Path | Becomes | Notes |
| --- | --- | --- |
| `instructions.md` | `SystemPromptSlots.role` | 主 Agent 的角色 slot，详见 [instructions.md](/docs/code/filesystem-instructions)。 |
| `agent.acl` | `CodeConfig` | 模型、provider、队列和目录发现配置，详见 [agent.acl](/docs/code/filesystem-config)。 |
| `skills/` | `skill_dirs` | AgentDir 私有技能，详见 [skills/ 技能目录](/docs/code/filesystem-skills)。 |
| `schedules/*.md` | `Vec<ScheduleSpec>` | 每个文件一条周期性 turn，详见 [schedules/ 调度目录](/docs/code/filesystem-schedules)。 |
| `tools/*.md` | `Vec<ToolSpec>` | 每个文件一个 `kind: mcp` 或 `kind: script` 工具，详见 [tools/ 工具目录](/docs/code/filesystem-tools)。 |
...
- AgentDir 是主 Agent 的目录；它不同于 `agent_dirs` / `registerAgentDir`，后者扫描 [agents/ 角色目录](/docs/code/filesystem-agents) 以获取 worker/subagent 定义。
