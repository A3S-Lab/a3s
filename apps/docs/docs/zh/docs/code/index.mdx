...
| TUI | [`a3s code`](/docs/code/tui) 将运行时事件流渲染为终端 transcript，并提供 tool activity、approval、memory、file panel 与 session controls。 |
| 文件系统优先 | [文件系统优先](/docs/code/filesystem-first) 把 `AGENTS.md`、`agent.acl`、`.a3s/agents/`、`skills/`、`tools/` 与 `schedules/` 组织成可审查的 Agent 产品接口。 |
...
| 命令 | [命令](/docs/code/commands) 区分 TUI slash commands 与 SDK command registry，后者用于宿主自定义 `/command` handler。 |
| 委派 | 内置角色与自定义 Markdown/YAML agents 可通过 `task`、`parallel_task`、`session.task(...)`、`session.tasks(...)` 和自动委派控制使用。 |
| 编排 | [`session.parallel`](/docs/code/orchestration)、`session.pipeline`、workflow phases、可恢复 checkpoint、loop caps 与 workflow budget ledger 支持确定性的宿主侧 fan-out。 |
| 定时服务 | [AgentDir](/docs/code/agent-dir) 可通过 `serveAgentDir` / `serve_agent_dir` 运行定时 turn，每个启用 schedule 对应一个稳定的 `schedule:<name>` session。 |
...
| Workspace | [Workspace 后端](/docs/code/workspace-backends) 让内置工具运行在本地文件、宿主提供的 workspace、可选 S3 兼容存储和可选 remote-git 服务上。 |
...
| 验证 | [验证](/docs/code/verification) 覆盖显式验证命令、预设、结构化 reports、summaries、artifacts、trace events 与 run replay 证据。 |
...
- [A3S Code TUI](/docs/code/tui) 说明安装、配置发现、slash commands，以及 CLI 与 runtime 的区别。
- [文件系统优先](/docs/code/filesystem-first)覆盖可审查的项目约定和 AgentDir 约定。
- [API 契约](/docs/code/api-contract)说明经过集成测试的 Node SDK surface。
- [会话](/docs/code/sessions)覆盖生命周期、streaming、run 状态、持久化与取消。
- [命令](/docs/code/commands) 区分 TUI 命令和 SDK command registry 中的自定义处理器。
- [工具](/docs/code/tools)覆盖直接工具、typed tool errors、结构化输出与 QuickJS programs。
- [验证](/docs/code/verification)覆盖命令证据、reports 与 summaries。
- [任务](/docs/code/tasks)覆盖模型驱动的子任务委派。
- [编排](/docs/code/orchestration)覆盖宿主驱动的 fan-out、pipeline 与可恢复 workflow。
- [安全](/docs/code/security)覆盖权限、确认、hooks 与验证 gate。
- [Hooks](/docs/code/hooks)覆盖工具、prompt、skill、session 与 error 生命周期拦截。
- [Agent Directory](/docs/code/agent-dir)覆盖持久化文件系统优先 agents、tools 与 schedules。
- [Memory](/docs/code/memory) 与 [持久化](/docs/code/persistence)覆盖可复用事实、session snapshot 与恢复路径。
- [Provider 配置](/docs/code/providers)覆盖 ACL provider 配置与环境变量注入。
