...
| 内置 OCR 路由 | `mcp__use_ocr__*` | 面向对象安全的 `OcrProvider` 接口。A3S release 默认注入基于 Power 原生 Rust 运行时的本地 `PP-OCRv6_small` provider；其他 provider 可声明不同的模型、可用性及数据是否离开设备，MCP annotations 会据此反映真实行为。 |
...
1. [文件系统优先](/docs/code/filesystem-first)
2. [会话](/docs/code/sessions)
3. [命令](/docs/code/commands)
4. [工具](/docs/code/tools)
5. [安全](/docs/code/security)
