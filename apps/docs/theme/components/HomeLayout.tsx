...
function MarkdownHome({ lang }: { lang: Lang }) {
  return (
    <main>
      ...
      <p>
        {lang === "cn"
          ? "每个项目都列出职责、交付阶段、当前版本或通道和代码入口。"
          : "Every project lists its responsibility, delivery stage, current version or channel, and source."}
      </p>
    </main>
  );
}
...
function MarkdownDownload({ lang }: { lang: Lang }) {
  return (
    <main>
      <h1>{lang === "cn" ? "下载 A3S" : "Download A3S"}</h1>
      <p>
        {lang === "cn"
          ? "获取适用于 macOS、Windows 和 Linux 的 A3S。"
          : "Get A3S for macOS, Windows, and Linux."}
      </p>
    </main>
  );
}
...
export function HomeLayout() {
  ...
  if (import.meta.env.SSG_MD) {
    return isDownloadPage
      ? <MarkdownDownload lang={lang} />
      : <MarkdownHome lang={lang} />;
  }

  return isDownloadPage
    ? <DesktopDownloadPage lang={lang} />
    : <HomePage lang={lang} />;
}
