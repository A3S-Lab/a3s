...
function MarkdownHome({ lang }: { lang: Lang }) {
  return (
    <main>
      ...
      <p>
        {lang === 'cn'
          ? 'A3S 包含 35 个职责明确的项目，覆盖编码、浏览器操作、持久执行、数据、评测与本地基础设施。'
          : 'A3S includes 35 focused projects for coding, browser work, durable execution, data, evaluation, and local infrastructure.'}
      </p>
      <h2>{lang === 'cn' ? '项目目录' : 'Project directory'}</h2>
      <p>{lang === 'cn' ? '每个项目都列出职责、交付阶段、发布通道和代码入口。' : 'Every project lists its responsibility, delivery stage, release channel, and source.'}</p>
