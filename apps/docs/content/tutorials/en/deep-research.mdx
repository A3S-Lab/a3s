...
description: Build a production-grade research agent with A3S Code — parallel web search across multiple machines, streaming synthesis, and structured report generation.
full: true
---
...
# Lane queue — controls parallel execution
queue {
...
async def parallel_search(session, sub_queries):
    print(f"📋 {len(sub_queries)} sub-queries planned")
        ...
        while not stop_event.is_set():
            try:
                ...
                for task in tasks:
                    ...
                    print(f"  📥 external task: {tid[:8]} ({kind})")
...
async def synthesize(session, question, results):
    ...
    for event in session.stream(prompt):
        if event.event_type == "text_delta":
            ...
            print(f"\n\n---\n✓ {event.total_tokens} tokens used")
            break
...
async def main():
    ...
    print(f"✓ {len(results)} searches completed")
