...
Pass a token budget to `parallel` so an entire fan-out shares **one ledger**.
Without a budget you get the plain outcomes; with one, `parallel` returns
`{ outcomes, budget }` and, once the cap is reached, a step that *starts*
afterward is denied. It is a soft cap — the in-flight fan-out is never
force-killed.
...
For the full programmable handle in Rust — `session.workflow()` with named,
resumable `phase`s, `WorkflowEvent` milestones, the `execute_loop` combinator,
and `budget_snapshot()` — see [Orchestration](/docs/code/orchestration).
