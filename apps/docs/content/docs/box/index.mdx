...
- **OCI image support** with pull, build, cache, and registry authentication; manifest digest exposed on every pulled image
- **Dockerfile build** with `ADD <url>` HTTP download and `ONBUILD` trigger inheritance from base images
...
- **Observability**: 19 Prometheus metrics, OpenTelemetry tracing spans, audit logging
