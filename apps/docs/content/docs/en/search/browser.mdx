...
[dependencies]
a3s-search = "0.9"  # 6 HTTP engines only

# With headless browser support (9 engines)
[dependencies]
a3s-search = { version = "0.9", features = ["headless", "lightpanda"] }
```

**Python/Node.js SDKs**: Headless support (Lightpanda) is enabled by default. Automatically downloads browser (~59MB) on first use.
