...
- **ACL Configuration** — Load engine weights, health settings, and timeouts from `.acl` config files.
- **Headless Browser** — Chrome/Chromium via `headless`, plus Lightpanda via `lightpanda` for JS-rendered engines (Google, Baidu, Bing China).
- **Auto-Install Browser** — Detects or downloads Chrome for Testing, or Lightpanda when the `lightpanda` feature is enabled.
...
- **Native SDKs** — TypeScript (NAPI-RS) and Python (PyO3) bindings with async support, headless browser enabled by default.
- **Cross-Platform** — macOS (arm64, x64), Linux (x64, arm64), and Windows (x64) with auto browser download.
...
<TypeTable
  type={{
    "Rust": {
      description: "Package: [a3s-search](https://crates.io/crates/a3s-search) · Install: a3s-search = '1.3' in Cargo.toml",
    },
    "Python": {
