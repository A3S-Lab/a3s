...
| `result_type` | `ResultType` | Type of result (see below) |
| `engines` | `HashSet<String>` | Engine shortcuts that found this result |
| `positions` | `Vec<u32>` | Position in each engine's result list |
| `score` | `f64` | Calculated consensus score |
| `thumbnail` | `Option<String>` | Thumbnail URL (if available) |
| `published_date` | `Option<String>` | Publication date (if available) |
