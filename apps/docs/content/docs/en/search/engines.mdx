...
| `shortcut` | `String` | — | Short identifier (e.g., `ddg`) |
| `categories` | `Vec<EngineCategory>` | `[General]` | Supported categories |
| `weight` | `f64` | `1.0` | Ranking weight multiplier |
| `timeout` | `u64` | `5` | Timeout in seconds |
| `enabled` | `bool` | `true` | Whether engine is active |
| `paging` | `bool` | `false` | Supports pagination |
| `safesearch` | `bool` | `false` | Supports safe search filtering |
...
| Shortcut | Aliases | Engine | Method |
|----------|---------|--------|--------|
| `ddg` | `duckduckgo` | DuckDuckGo | HTTP |
| `brave` | — | Brave Search | HTTP |
| `bing` | — | Bing International | HTTP |
| `wiki` | `wikipedia` | Wikipedia | JSON API |
| `sogou` | — | Sogou | HTTP |
| `360` | `so360` | 360 Search | HTTP |
| `g` | `google` | Google | Headless |
| `baidu` | — | Baidu | Headless |
| `bing_cn` | — | Bing China | Headless |
