...
Headless browser engines (Google, Baidu, Bing China) are **opt-in** for Rust library. HTTP-only (default):

```toml
a3s-search = "1.3"  # 6 HTTP engines only
```

With Chrome/Chromium headless browser support:

```toml
a3s-search = { version = "1.3", features = ["headless"] }  # 9 engines
```

With Lightpanda support on Linux/macOS, enable `lightpanda`; it implies `headless`:
...
Python 3.9–3.13. Pre-built wheels for Linux (glibc/musl), macOS, and Windows. **Headless browser support (Lightpanda) is enabled by default** — automatically downloads browser (~59MB) on first use.
...
Node.js 16+. Native binary for Linux, macOS, and Windows (x64/arm64). **Headless browser support (Lightpanda) is enabled by default** — automatically downloads browser (~59MB) on first use.
...
macOS and Linux. Installs the `a3s-search` CLI binary. See [CLI](/docs/search/cli) for usage.
