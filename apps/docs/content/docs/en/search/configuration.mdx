---
title: Configuration
description: ACL configuration files and health monitoring for engine management
---
...
A3S Search supports ACL configuration files for engine settings and health monitoring. Load settings from a `.acl` file instead of hardcoding them.
...
<TypeTable
  type={{
    "SearchConfig::load(path)": {
      description: "Load config from .acl file",
    },
    "SearchConfig::parse(content)": {
      description: "Parse ACL string",
    },
    "health_config()": {
...
Combine ACL config with health monitoring:
