---
title: "A3S Code"
description: "Harness-driven coding agent runtime with the v4.1.0 SDKs"
---

# A3S Code

A3S Code v4.1.0 is a harness-driven runtime for coding agents. The runtime keeps the agent loop small and observable while the harness owns context assembly, tool execution, permission policy, delegation, verification evidence, and run replay.

## v4.1.0 Highlights

| Area | Current capability |
| --- | --- |
| Filesystem-first | [Filesystem-first](/docs/code/filesystem-first) organizes `AGENTS.md`, `agent.acl`, `.a3s/agents/`, `skills/`, `tools/`, and `schedules/` into a reviewable, versioned agent product interface. |
...
| Role directories | Custom Markdown/YAML worker agents load from configured `agent_dirs`, project/user `.a3s/agents`, and Claude-compatible `.claude/agents` migration paths. `.a3s/agents` is the native A3S location and wins for same-name agents in the same scope. |
...
| Workspace stack | [Workspace Backends](/docs/code/workspace-backends) expose local filesystem, S3-compatible object storage, and optional HTTP/JSON remote-git backends through the SDKs. |
...
- **Workspace backends**: `LocalWorkspaceBackend`, `S3WorkspaceBackend`, and `remoteGit` let hosts run the same agent loop against local files or object-storage workspaces.
