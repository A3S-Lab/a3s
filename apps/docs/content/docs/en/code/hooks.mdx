...
| `working_directory` | `String` | Current workspace path |
| `recent_tools` | `Vec<String>` | Recently called tools |
...
| `args` | `Value` | Tool arguments |
| `result.success` | `bool` | Whether tool succeeded |
| `result.output` | `String` | Tool output text |
| `result.exit_code` | `i32` | Exit code |
| `result.duration_ms` | `u64` | Execution time |
...
| `available_tools` | `Vec<String>` | Registered tool names |
...
| `tool_calls` | `Vec<String>` | Tools called this turn |
| `usage.total_tokens` | `u32` | Token usage |
| `duration_ms` | `u64` | Generation time |
...
|--------|------|-------------|
| Register hook | `session.register_hook(Hook::new(id, event_type))` | Register a hook listener |
| Register handler | `session.register_hook_handler(id, Arc::new(handler))` | Attach handler to hook |
| Unregister | `session.unregister_hook(id)` | Remove hook and handler |
| Count | `session.hook_count()` | Number of registered hooks |
...
| `HookResponse::block(reason)` | Cancel the operation with a reason string |
| `HookResponse::modify(data)` | Replace event data before execution |
