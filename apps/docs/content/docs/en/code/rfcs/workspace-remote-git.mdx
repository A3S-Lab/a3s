...
## 15. Implementation Plan (Phase 4.2 Sketch)

Assuming this RFC is accepted, Phase 4.2 lands in this order:

1. Add `remote-git` cargo feature and `reqwest` is already in deps —
   verify `rustls`/`native-tls` choice matches the rest of the crate.
2. New module `crates/code/core/src/workspace/remote_git.rs` with
   `RemoteGitBackend` + `RemoteGitBackendConfig` + `RemoteGitConflict`.
3. Implement `WorkspaceGit` and `WorkspaceGitStashProvider`.
4. Add `WorkspaceServices::with_remote_git` and (optional)
   `s3_with_remote_git`.
5. Unit tests: per-method request shape (using `wiremock`),
   per-error-code conflict mapping, timeout / metering events.
6. Integration test fixture: tiny `wiremock`-backed server that exercises
   the full surface end-to-end through the `git` tool.
7. README + `code/sessions.mdx` section: how to wire remote git;
   trade-offs vs local.
8. CHANGELOG entry.

No SDK-binding exposure in Phase 4.2 — the Node and Python SDKs can land
the remote git option in a follow-up once the trait is settled.
