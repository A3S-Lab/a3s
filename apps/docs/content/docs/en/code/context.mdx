...
<Callout type="info">
  ...
  memory-backed context by passing a `FileMemoryStore` instance — the provider is activated
  automatically.
</Callout>
...
<TypeTable
  type={{
    path: {
      type: 'PathBuf',
      required: true,
      description: 'Workspace root. Use `RipgrepContextConfig::new(path)`',
    },
    case_insensitive: {
      type: 'bool',
      default: 'true',
      description: 'Case-insensitive search. Use `.with_case_insensitive(b)`',
    },
    context_lines: {
      type: 'usize',
      default: '2',
      description: 'Context lines around matches. Use `.with_context_lines(n)`',
    },
    max_file_size: {
      type: 'usize',
      default: '1MB',
      description: 'Max file size to search. Use `.with_max_file_size(n)`',
    },
  }}
/>
