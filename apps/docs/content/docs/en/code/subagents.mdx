...
<TypeTable
  type={{
    ...
    system_prompt: { type: 'string', description: "Agent's system prompt / instructions (legacy — maps to `extra` slot)" },
    prompt: { type: 'string', description: "Agent's prompt / instructions" },
