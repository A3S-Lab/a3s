...
| `Procedural` | Successful patterns (auto-stored on turn complete) | 0.8 |
...
| Event | Description |
|-------|-------------|
| `MemoryStored` | A memory item was persisted |
| `MemoryRecalled` | A memory item was retrieved for context |
| `MemoriesSearched` | A search was performed |
| `MemoryCleared` | All memories were cleared |
...
| Method | Description |
|--------|-------------|
| `.with_file_memory(path)` | Enable `FileMemoryStore` at the given path |
| `.with_memory(Arc<dyn MemoryStore>)` | Use a custom backend |
| `.with_memory_config(MemoryConfig)` | Configure relevance scoring and tier capacities |

### MemoryStore trait

| Method | Description |
|--------|-------------|
| `store(item)` | Persist a memory item (upsert by ID) |
| `retrieve(id)` | Fetch a single item by ID |
| `search(query, limit)` | Substring search, ranked by relevance |
| `search_by_tags(tags, limit)` | Filter by tags, ranked by relevance |
| `get_recent(limit)` | Most recently created items |
| `get_important(threshold, limit)` | Items above importance threshold, sorted descending |
