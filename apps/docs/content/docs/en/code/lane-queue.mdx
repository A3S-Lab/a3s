...
|-------|------|---------|-------------|
| `query_max_concurrency` | `usize` | `4` | Max parallel Query-lane tasks |
| `execute_max_concurrency` | `usize` | `1` | Max parallel Execute-lane tasks |
| `enable_metrics` | `bool` | `false` | Collect queue metrics |
| `enable_dlq` | `bool` | `false` | Enable dead-letter queue |
| `lane_handlers` | `HashMap<SessionLane, LaneHandlerConfig>` | `{}` | Per-lane handler config |
| `retry_policy` | `Option<RetryPolicy>` | `None` | Retry on failure |
| `rate_limit` | `Option<RateLimit>` | `None` | Rate limiting |
| `priority_boost` | `Option<PriorityBoost>` | `None` | Deadline-based priority boost |
| `pressure_threshold` | `usize` | `50` | Queue depth alert threshold |

### LaneHandlerConfig fields

| Field | Type | Description |
|-------|------|-------------|
| `mode` | `TaskHandlerMode` | `Internal`, `External`, or `Hybrid` |
| `timeout_ms` | `Option<u64>` | External task timeout |
...
| Set lane handler | `session.set_lane_handler(lane, cfg).await` | `await session.set_lane_handler(lane, cfg)` | `await session.setLaneHandler(lane, cfg)` |

### RetryPolicy fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `strategy` | `string` | `"exponential"` | `"fixed"` or `"exponential"` |
| `max_retries` | `u32` | `3` | Max retry attempts |
| `initial_delay_ms` | `u64` | `100` | Initial retry delay |
