...
Custom command registration is not yet supported in the Node.js SDK. Use built-in slash commands via `send()`.
...
<TypeTable
  type={{
    ...
    "Register command": {
      description: "Python: session.register_command(name, desc, handler) · TypeScript: not yet supported",
    },
    "Schedule task": {
