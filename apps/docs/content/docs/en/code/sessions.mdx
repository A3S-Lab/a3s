...
<TypeTable
  type={{
    ...
    tool_timeout_ms: {
      type: 'Option<u64>',
      description: 'Per-tool execution timeout in ms; timeout → error fed back to LLM. Rust: `with_tool_timeout(ms)`, TS: `toolTimeoutMs: ms`, Python: `tool_timeout_ms=ms`',
    },
    circuit_breaker: {
      type: 'u32',
      default: '3',
    ...
    prompt_slots: {
      ...
      description: 'Slot-based system prompt customization. Rust: `with_prompt_slots(slots)`, TS: `role/guidelines/responseStyle/extra`, Python: `opts.role/guidelines/response_style/extra`',
    },
  }}
/>
...
// Interval at the end ("every" clause)
let r = session.send("/loop monitor errors every 1m", None).await?;

// Default interval (10 minutes when omitted)
...
<TypeTable
  type={{
    ...
    "List commands": {
      description: "Rust: registry.list_full() via session.command_registry() · Python: session.list_commands() · TypeScript: session.listCommands()",
    },
    "Register command": {
      description: "Rust: session.register_command(Arc::new(cmd)) · Python: session.register_command(name, desc, handler) · TypeScript: not yet supported",
    },
    "Schedule task": {
