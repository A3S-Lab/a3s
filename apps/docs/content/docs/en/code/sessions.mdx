...
While a stream is active, `currentRun()` returns the active run snapshot:

```ts
const active = await session.currentRun();
if (active?.id) await session.cancelRun(active.id);
```

Python exposes the same surface as `runs()`, `run_snapshot(run_id)`,
`run_events(run_id)`, `current_run()`, and `cancel_run(run_id)`.
