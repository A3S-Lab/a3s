...
## BTW (By The Way) - Ephemeral Side Questions

The `btw()` method lets you ask ephemeral side questions without affecting the conversation history. This is useful for quick lookups, clarifications, or context checks that shouldn't be part of the main conversation flow.
...
- Uses a simplified system prompt optimized for quick answers
...
// Continue main conversation (btw question not in history)
...
<Callout type="warn">
**Limitations:**
- No tool execution (answers are based on LLM knowledge + conversation context only)
- Not suitable for tasks requiring file access or code execution
- Uses a simplified system prompt (no agent persona or custom instructions)
</Callout>
