...
The integration check also covers the ACL field that creates a file-backed
session store:
...
`storage_url` is parsed for custom storage metadata, but it is not the local
file-session persistence path. Use `sessions_dir` in ACL or pass
`sessionStore` in SDK options.
...
> Full guide: [Tools](/docs/code/tools).
...
> Full guide: [Verification](/docs/code/verification).
...
> Full guide: [Memory](/docs/code/memory).
...
> Full guide: [Skills](/docs/code/skills).
...
> Full guide: [Sessions](/docs/code/sessions).

The current SDK surface has no `session.btw()` helper. Use explicit history
when you want a side question that does not mutate the session transcript:
...
> Full guide: [Persistence](/docs/code/persistence) and [Sessions](/docs/code/sessions).
...
> Full guide: [Tasks](/docs/code/tasks) and [Orchestration](/docs/code/orchestration).
...
> Full guide: [Hooks](/docs/code/hooks).
...
> Full guide: [Commands](/docs/code/commands).
...
> Full guide: [Lane Queue](/docs/code/lane-queue).
...
> Full guide: [MCP](/docs/code/mcp). Idle disconnect: [Cluster Extension Points](/docs/code/cluster-extension-points).
...
const count = await session.addMcpServer(
  ...
  'stdio',
  process.execPath,
  ['tools/mcp_echo_server.mjs', 'example-value'],
);
...
new HttpTransport(
  requiredEnv('AHP_HTTP_ENDPOINT'),
  process.env.AHP_AUTH_TOKEN,
).kind; // 'http'
new WebSocketTransport(
  requiredEnv('AHP_WS_ENDPOINT'),
  process.env.AHP_AUTH_TOKEN,
).kind; // 'websocket'
...
> Full guide: [Cluster Extension Points](/docs/code/cluster-extension-points) (identity labels, budget guard, cluster events, deterministic IDs/replay, loop checkpoints, retention caps).

These contracts let a cluster control plane wire
...
as `retentionLimits`; Python exposes it as `opts.retention_limits`.
...
arguments and catch exceptions as Allow.
