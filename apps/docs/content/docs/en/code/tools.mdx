...
v2.1.0 keeps a registry of available tools but exposes only the useful subset to the model for the current turn. This reduces prompt clutter and keeps side effects behind one permission path.
...
`web_fetch`, `web_search`, `git`, `batch`, and MCP tools are made visible when the turn requires them.

## Direct Tool Calls

SDK callers can bypass the model for deterministic operations:
...
Direct calls still execute inside the session's workspace and effective security policy.
...
The QuickJS VM receives no filesystem, network, subprocess, or environment permissions. The only useful capabilities are the `ctx` methods wired back to A3S Code tools. PTC output should be structured: summary, findings, evidence refs, artifacts, risks, and suggested next actions. Raw output belongs in traces or artifacts, not repeatedly in the prompt.

## Tool Results

`ToolResult` includes `name`, `output`, `exitCode`, and optional structured metadata. Long outputs should be preserved as trace or artifact data and summarized before they are fed back to the model.
