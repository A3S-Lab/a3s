...
description: 15 built-in tools (16 with sandbox) for file operations, shell execution, search, web access, git worktree, and subagent delegation
---
...
A3S Code ships with 15 built-in tools (16 with the `sandbox` feature) covering file operations, shell execution, search, web access, git worktree, and subagent delegation. All tools execute in-process via native Rust implementations.
