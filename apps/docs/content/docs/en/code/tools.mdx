...
|-----|------|----------|-------------|
| `file_path` | `string` | ✅ | Absolute or workspace-relative path |
...
| Arg | Type | Required | Description |
|-----|------|----------|-------------|
| `file_path` | `string` | ✅ | Path to patch |
...
| `prompt` | `string` | ❌ | Extraction prompt for the fetched content |
...
| `workspace` | `string` | ❌ | Workspace override (default: current) |

#### batch

| Arg | Type | Required | Description |
|-----|------|----------|-------------|
| `invocations` | `array` | ✅ | List of `{tool, args}` objects to run in parallel |
