...
**Agent Harness Protocol (AHP) 2.2** is a universal, transport-agnostic protocol for supervising autonomous AI agents. A3S Code provides native AHP 2.2 integration through the hook system, enabling external supervision, policy enforcement, and **context-aware** security controls.

<Callout type="info">
AHP 2.2 provides **18 harness points** including PreAction, PostAction, ContextPerception, Idle, Confirmation, MemoryRecall, and more. ContextPerception enables intent-driven context injection based on what the user is actually trying to do.
</Callout>

## Overview

AHP 2.2 provides a framework-agnostic interface for:
...
- **Context perception** — Inject relevant context based on user intent
- **Idle detection** — Trigger background consolidation when agent is idle
...
## AHP 2.2 Harness Points

AHP 2.2 defines **18 harness points** across the agent lifecycle:

| Harness Point | Type | Description |
|---------------|------|-------------|
| `PreAction` | Blocking | Before tool execution — block, allow, or modify |
| `PostAction` | Non-blocking | After tool execution — sanitize output |
| `PrePrompt` | Blocking | Before LLM prompt — inject context, modify prompt |
| `PostResponse` | Non-blocking | After LLM response — sanitize, filter |
| `PreContextPerception` | Blocking | Before context injection — harness can inject context |
| `PostContextPerception` | Non-blocking | After context resolution — enrich or filter |
| `SessionStart` | Non-blocking | Session initialized — setup, policy load |
| `SessionEnd` | Non-blocking | Session ended — cleanup, persist state |
| `Idle` | Blocking | Agent idle — background consolidation (dream system) |
| `Confirmation` | Blocking | Ambiguous operation — human approval required |
| `Query` | Blocking | External query — harness provides guidance |
| `Heartbeat` | Non-blocking | Periodic health check — status update |
| `Error` | Non-blocking | Error occurred — error handling, recovery |
| `Success` | Non-blocking | Task completed — success handling |
| `MemoryRecall` | Blocking | Memory query — harness provides facts |
| `Planning` | Blocking | Task planning — harness validates or modifies plan |
| `Reasoning` | Blocking | Reasoning step — harness provides hints |
| `RateLimit` | Non-blocking | Rate limit hit — backoff, retry strategy |

### Context Perception Flow

```
User Prompt → Intent Detection (~7µs)
                    ↓
        ┌───────────────────────────┐
