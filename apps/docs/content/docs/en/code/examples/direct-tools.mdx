...
instead of an agent turn. For unattended calls, prefer an explicit allow-list
with a closed fallback so this example cannot grow into arbitrary tool access.
...
const session = agent.session('.', {
  permissionPolicy: {
    allow: ['glob(*)', 'grep(*)', 'read(*)', 'generate_object(*)'],
    deny: ['write(*)', 'bash(*)'],
