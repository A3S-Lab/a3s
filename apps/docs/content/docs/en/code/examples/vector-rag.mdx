...
result = session.send("Find all JWT authentication code")
print(result.text)
```
</Tab>
</Tabs>
...
result = session.send("Search for code related to JWT token verification")
print(result.text)
```
</Tab>
</Tabs>
...
<TypeTable
  type={{
    "Filesystem context": {
      description: "Rust: .with_fs_context(path) · Python: fs_context=path · TypeScript: fsContext: path · Default: None",
    },
    "Custom context provider": {
      description: "Rust: .with_context_provider(Arc::new(p)) · Python: _(config-based)_ · TypeScript: _(config-based)_ · Default: None",
    },
  }}
/>
