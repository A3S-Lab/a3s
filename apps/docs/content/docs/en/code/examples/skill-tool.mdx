...
The **Skill tool** (introduced in v1.3.4) allows skills to be invoked as first-class tools with temporary permission grants. This enforces skill-based access patterns and prevents agents from bypassing skills to directly access underlying tools.

<Callout type="info">
**GitHub Issue:** [#8 - Skill Tool mechanism for enforcing skill-based tool access](https://github.com/A3S-Lab/Code/issues/8)
</Callout>
...
const session = agent.session('.', {
  skillsDir: './skills',
  permissive: true  // Allow automatic tool execution
});
...
See the [test results](/docs/code/examples/skill-tool-test) for a complete test with the Kimi model.
