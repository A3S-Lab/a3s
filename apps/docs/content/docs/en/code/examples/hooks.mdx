...
**Run:** `cargo run --example 08_hooks`
**Source:** [`core/examples/08_hooks.rs`](https://github.com/A3S-Lab/Code/blob/main/core/examples/08_hooks.rs)
</Tab>
<Tab value="Python">
```python
tool_log = []
    ...
    fn handle(&self, event: &HookEvent) -> HookResponse {
        ...
        for pat in BLOCKED {
            if cmd.contains(&pat.to_lowercase()) {
                eprintln!("⛔  BLOCKED: '{pat}' in command");
                return HookResponse::block(format!("[sentinel] '{pat}'"));
            }
        }
        HookResponse::continue_()
    }
...
// Sentinel propagates to every sub-agent spawned by the task tool
let result = session.send("Analyze the repo with sub-agents", None).await?;
println!("{}", result.text);
```
</Tab>
</Tabs>
