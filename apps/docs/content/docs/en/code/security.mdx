...
use a3s_code_core::security::default::{DefaultSecurityProvider, DefaultSecurityConfig, SensitivePattern};
use std::sync::Arc;

// Activate with defaults (all features on)
let session = agent.session("/project", Some(
...
let config = DefaultSecurityConfig {
    ...
    custom_patterns: vec![
        SensitivePattern::new("internal_id", r"INT-\d{6}", "REDACTED:ID"),
    ],
};
let provider = Arc::new(DefaultSecurityProvider::with_config(config));
let session = agent.session("/project", Some(
    SessionOptions::new().with_security_provider(provider)
))?;
```
</Tab>
<Tab value="TypeScript">
...
```python
# Default security config
session = agent.session("/project", default_security=True)
