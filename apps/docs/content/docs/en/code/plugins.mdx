---
title: Plugin System
description: Opt-in tool+skill bundles — mount agentic_search, agentic_parse, or build your own plugin
---
...
Plugins are **opt-in**. No plugin tools are loaded by default — you choose exactly which capabilities each session needs.

## Built-in Plugins

| Plugin class | Tool | Companion skill |
|---|---|---|
| `AgenticSearch` | `agentic_search` — multi-phase semantic code search | `agentic-search` — guides usage, modes, and best practices |
| `AgenticParse` | `agentic_parse` — LLM-enhanced document parsing | `agentic-parse` — guides parse strategies and extraction |

For parameters, search modes, and examples see the [Agentic Search deep dive](/docs/code/examples/agentic-search).
