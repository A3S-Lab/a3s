...
let session = agent.session("/project", Some(
    SessionOptions::new()
        .with_max_tool_rounds(20)  // default 50
))?;
```
</Tab>
...
let session = agent.session("/project", Some(
    SessionOptions::new()
        .with_tool_timeout_ms(10_000)  // max 10s per tool
))?;
```
</Tab>
<Tab value="TypeScript">
...
let session = agent.session("/project", Some(
    SessionOptions::new()
        .with_circuit_breaker_threshold(5)  // default 3
))?;
```
</Tab>
<Tab value="TypeScript">
...
// Default: 3 retries, 1s base, 30s max, ±25% jitter
...
let session = agent.session("/project", Some(
    SessionOptions::new()
        .with_max_parse_retries(3)  // default 2
))?;
```
</Tab>
<Tab value="TypeScript">
...
let session = agent.session("/project", Some(
    SessionOptions::new()
        .with_continuation(true)           // enabled by default
        .with_max_continuation_turns(5)    // default 3
))?;

// Disable continuation entirely
let session = agent.session("/project", Some(
    SessionOptions::new()
...
let session = agent.session("/project", Some(
    SessionOptions::new()
        ...
        .with_auto_compact_threshold(0.75)  // trigger at 75% (default 80%)
))?;
```
</Tab>
<Tab value="TypeScript">
