...
// Build an McpManager (or reuse the one from AgentConfig)
let manager = Arc::new(McpManager::new());

// Connect a filesystem MCP server and register its tools
...
// Connect a filesystem MCP server at runtime
...
// Connect a GitHub MCP server with env vars
...
<TypeTable
  type={{
    ...
    command: { type: 'string', required: true, description: 'Executable to launch (e.g. `npx`, `uvx`, `/usr/local/bin/my-server`)' },
    args: { type: 'string[]', description: 'Command-line arguments passed to the server process' },
