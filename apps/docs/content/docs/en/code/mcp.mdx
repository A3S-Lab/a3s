...
A3S Code supports three MCP transports:
...
<TypeTable
  type={{
    ...
    "http": {
      ...
      description: "Remote servers, shared infrastructure (legacy SSE transport)",
    },
    "streamable-http": {
      type: "JSON-RPC 2.0 over HTTP (MCP 2025-03-26)",
      description: "Remote servers — single endpoint, supports both JSON and SSE responses, session management via Mcp-Session-Id header",
    },
  }}
/>
...
<TypeTable
  type={{
    ...
    transport: { type: 'string', default: '"stdio"', description: '`stdio`, `http`, or `streamable-http`' },
    command: { type: 'string', description: 'Executable to launch (stdio only)' },
    args: { type: 'string[]', description: 'Command arguments (stdio only)' },
    url: { type: 'string', description: 'Server endpoint URL (http / streamable-http only)' },
    headers: { type: 'object', description: 'HTTP headers (http / streamable-http only)' },
...
println!("Registered {tool_count} tools from filesystem server");

// The tools are now available: mcp__filesystem__read_file, etc.
let result = session.send("List files in /tmp").await?;
```
</Tab>
...
<TypeTable
  type={{
    name: { type: 'string', required: true, description: 'Server identifier — used as the tool name prefix (`mcp__{name}__{tool}`)' },
    transport: { type: "'stdio' | 'http' | 'streamable-http'", default: '"stdio"', description: 'Transport protocol' },
    command: { type: 'string', description: 'Executable to launch — required for `stdio` transport' },
    args: { type: 'string[]', description: 'Command-line arguments passed to the server process (stdio only)' },
    url: { type: 'string', description: 'Server endpoint URL — required for `http` and `streamable-http` transports' },
    headers: { type: 'object', description: 'HTTP headers sent with every request (http / streamable-http only)' },
    env: { type: 'object', description: 'Extra environment variables for the server process (stdio only)' },
  }}
/>
