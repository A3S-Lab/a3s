...
# Optional pre-warm or repair of the pinned local PP-OCRv6 models.
a3s install use/ocr
...
# The first extraction installs the models automatically when policy allows.
...
“Use Office to update this workbook,” or “Use OCR on this scan.” If a
...
inside the Use worker only after parent TUI confirmation. Native Office needs
no provider. If an unpromoted operation requires the optional compatibility
route, the worker requests `office_install_compat` through parent confirmation
and uses the hot-plugged route once it is ready. OCR follows the same first-use
pattern: after a read-only doctor reports missing or damaged models, the worker
requests the bounded `ocr_install` tool through parent confirmation and
continues to extraction after installation succeeds. Direct Browser launches,
Office compatibility commands, and OCR extraction prepare their respective
shared or pinned assets without requiring a separate install command.
