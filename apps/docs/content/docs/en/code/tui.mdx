...
| Built-in OCR route | `mcp__use_ocr__*` | Uses the object-safe `OcrProvider` contract. A3S releases inject the local `PP-OCRv6_small` ONNX provider by default; other providers can declare different model, availability, and off-device behavior, which is reflected in MCP annotations. |
