...
<TypeTable
  type={{
    "Custom store": {
      description: "Python: _(Rust only)_ · TypeScript: _(Rust only)_ · Default: FileSessionStore",
    },
  }}
/>
...
<TypeTable
  type={{
    ...
    'config.system_prompt': { type: 'String', description: 'Assembled system prompt (built from prompt slots)' },
...
<TypeTable
  type={{
    "session.save()": {
      description: "Persist current session state to the configured store",
    },
    "agent.resumeSession(id, options)": {
      description: "Resume a saved session by ID (requires sessionStore in options)",
    },
    "session.sessionId": {
      description: "Property: Get the current session's unique ID",
    },
  }}
/>
