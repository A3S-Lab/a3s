...
MemoryType                      SqliteMemoryStore (feature-gated)
RelevanceConfig                 (your custom backend)
PrunePolicy
```
...
SQLite storage is feature-gated:
...
<TypeTable
  type={{
    ...
    "PrunePolicy": {
      description: "Automatic pruning parameters — max age, minimum importance to keep, hard item cap",
    },
    "InMemoryStore": {
    ...
    "SqliteMemoryStore": {
      description: "Feature-gated persistent backend — SQLite WAL, FTS5 BM25 search, Markdown export, session log",
    },
  }}
/>
