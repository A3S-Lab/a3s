...
<TypeTable
  type={{
    "A3S_IMAGE_CACHE_SIZE": {
      description: "Override max image cache size in bytes",
    },
    "A3S_TEE_SIMULATE": {
      description: "Enable TEE simulation mode (development only)",
    },
    "A3S_DEPS_STUB": {
      description: "Stub out libkrun for CI builds without KVM",
    },
    "RUST_LOG": {
