...
|-------|-------------|---------|
| `pids_limit` | Max processes (RLIMIT_NPROC) | `1000` |
| `cpuset_cpus` | Pin to specific CPUs | `"0,1,3"` |
| `cpu_shares` | cgroup v2 cpu.weight | `512` |
| `cpu_quota` | cgroup v2 cpu.max (microseconds) | `50000` |
| `cpu_period` | cgroup v2 period (default 100000 us) | `100000` |
| `memory_reservation` | cgroup v2 memory.low | `536870912` |
| `memory_swap` | cgroup v2 memory.swap.max (-1 = unlimited) | `-1` |
| `ulimits` | Custom rlimits (RESOURCE=SOFT:HARD) | `"nofile=1024:4096"` |
...
| `max-file` | `3` | Number of rotated files to keep (gzip compressed) |
...
| Option | Default | Description |
|--------|---------|-------------|
| `syslog-address` | `udp://127.0.0.1:514` | Syslog server address (`udp://` or `tcp://`) |
| `syslog-facility` | `daemon` | Syslog facility |
| `tag` | box short ID | Message tag |
