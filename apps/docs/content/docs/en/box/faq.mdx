...
Use macOS Apple Silicon for local HVF development or Linux hosts with KVM/libkrun support for Linux validation. Windows runtime code exists, but the documented and tested product path is macOS/Linux. Windows CRI is not supported.
