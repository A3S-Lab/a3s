...
A3S Gateway is the traffic layer for AI-native services. It is built on [hyper](https://hyper.rs/) and [rustls](https://github.com/rustls/rustls), ships as a single statically-linked binary, and is configured exclusively in ACL (Agent Configuration Language).
...
Hot reload replaces the router table and service registry atomically through a shared runtime snapshot. HTTP/TCP entrypoints are not rebound when listener configuration is unchanged. Added or moved HTTP/TCP listeners are reconciled by name, and unchanged listeners stay active if a new bind fails. UDP entrypoints restart explicitly.
...
<TypeTable
  type={{
    "HTTP/1.1 & HTTP/2": {
      description: "Full reverse proxy with hop-by-hop header filtering, normalized X-Forwarded-* metadata, and connection pooling",
    },
    "SSE / Streaming": {
...
- Forwarded headers — normalized `X-Forwarded-For`, `X-Forwarded-Host`, `X-Forwarded-Proto`, and `X-Forwarded-Port` on upstream HTTP requests
...
- ACL-only configuration — consistent, human-readable, version-controlled
- CLI-first management with optional authenticated/mTLS Dashboard API on a dedicated listener
