...
<TypeTable
  type={{
    strategy: { type: 'string', default: '"round-robin"', description: 'Load balancing strategy' },
    request_timeout: { type: 'string', default: '"30s"', description: 'Maximum buffered plain HTTP upstream request time before a 504 response' },
    servers: { type: 'array', required: true, description: 'Backend URLs and optional weights' },
  }}
/>
