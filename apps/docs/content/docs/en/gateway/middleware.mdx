...
|------|---------|-------------|
| `jwt` | 401 | JWT Bearer token validation |
| `api-key` | 401 | Header-based API key check |
| `basic-auth` | 401 | HTTP Basic authentication |
| `forward-auth` | 401 | External auth delegation |
| `rate-limit` | 429 | In-memory token bucket |
| `rate-limit-redis` | 429 | Distributed token bucket (Redis) |
| `cors` | — | CORS headers + preflight |
| `headers` | — | Request/response header manipulation |
| `strip-prefix` | — | Path prefix removal |
| `body-limit` | 413 | Max request body size |
| `retry` | — | Retry on backend failure |
| `circuit-breaker` | 503 | Closed/Open/HalfOpen state machine |
| `ip-allow` | 403 | CIDR/IP allowlist |
| `compress` | — | brotli/gzip/deflate compression |
| `tcp-filter` | — | TCP connection limit + IP filter |
