...
<TypeTable
  type={
    {`\`TriggerFilter::by_category("orders")\``}: {
      description: {`Events with \`category == "orders"\``},
    },
    {`\`TriggerFilter::by_event_type("order.created")\``}: {
      description: {`Events with \`event_type == "order.created"\``},
    },
    {`\`TriggerFilter::by_subject_prefix("events.market.")\``}: {
      description: "Events whose subject starts with the prefix",
    },
  }
/>
...
<TypeTable
  type={
    {`\`TopicSink\``}: {
      description: {`Re-publishes events to an \`EventProvider\` topic (fan-out)`},
    },
    {`\`InProcessSink\``}: {
      description: "Calls an async handler function",
    },
    {`\`LogSink\``}: {
      description: {`Logs events via \`tracing\` (debugging)`},
    },
    {`\`CollectorSink\``}: {
      description: "Collects events in memory (testing)",
    },
    {`\`FailingSink\``}: {
      description: "Always fails delivery (testing error paths)",
    },
  }
/>
