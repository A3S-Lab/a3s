...
The `a3s-code` package provides native PyO3 bindings to `a3s-code-core`. Synchronous API — all calls block until completion.
...
The Python SDK uses synchronous blocking calls via `py.allow_threads(|| runtime.block_on(...))`. This is the standard PyO3 pattern — the GIL is released during Rust execution, so other Python threads can run concurrently.
