...
| `timeout_ms` | `Option<u64>` | Max wait for external worker response |
...
| `payload` | `Value` | Task data (prompt, tool args, etc.) |
| `created_at` | `DateTime<Utc>` | Enqueue timestamp |
| `timeout_ms` | `Option<u64>` | Per-task timeout override |
...
| `duration_ms` | `Option<u64>` | Worker execution time |
