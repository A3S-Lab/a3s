...
|--------|------|--------|---------|---------|
| Filesystem context | `.with_fs_context(path)` | `fs_context=path` | `fsContext: path` | `None` |
| Custom context provider | `.with_context_provider(Arc::new(p))` | _(config-based)_ | _(config-based)_ | `None` |
...
| `.with_min_relevance(f)` | Minimum cosine similarity score | `0.3` |
| `.with_max_results(n)` | Max chunks returned per query | `10` |
