...
1. **Set token budgets** — Use `max_tokens` to prevent context overflow
2. **Filter by type** — Use `context_types` to retrieve only relevant context
3. **Adjust depth** — Use `Abstract` for summaries, `Full` for detailed content
4. **Implement relevance scoring** — Return items sorted by relevance (0.0–1.0)
5. **Use on_turn_complete** — Extract and store information from conversations
6. **Cache results** — Implement caching in your provider to reduce latency
