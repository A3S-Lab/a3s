...
<Steps>
<Step>**During generation** — The agent loop queries the memory system for relevant context before each LLM call</Step>
<Step>**After tool execution** — Important observations (e.g., project structure, user preferences) can be stored as memories</Step>
<Step>**Across turns** — Working memory tracks the current task state within a session</Step>
<Step>**Across sessions** — Episodic and semantic memories persist via `FileMemoryStore`, available when a session is restored</Step>
</Steps>
