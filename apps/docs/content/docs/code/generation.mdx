...
The generation APIs send prompts to the LLM and return responses. The agent loop handles tool execution automatically — you just call `send()` or `stream()`.
...
for (const event of events) {
  ...
  if (event.type === 'tool_end') console.log(`  → ${event.toolOutput?.slice(0, 200)}`);
