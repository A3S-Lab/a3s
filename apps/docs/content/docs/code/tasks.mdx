...
| Max tool rounds | `.with_max_tool_rounds(n)` | `max_tool_rounds=n` | `maxToolRounds: n` | `50` |
...
| `status` | `TaskStatus` | `Pending`, `InProgress`, `Completed`, `Failed`, `Cancelled` |
| `priority` | `TaskPriority` | `Critical`, `High`, `Medium`, `Low` |
| `dependencies` | `Vec<String>` | IDs of tasks that must complete first |
...
| `success_criteria` | `Vec<String>` | Measurable completion criteria |
| `progress` | `f32` | Progress (0.0–1.0) |
| `achieved` | `bool` | Whether goal is complete |
| `created_at` | `DateTime<Utc>` | Creation timestamp |
...
|--------|-----------|-------------|
| `plan` | `async fn plan(&self, prompt: &str, context: &PlanContext) -> Result<Vec<AgentTask>>` | Decompose prompt into tasks |
| `name` | `fn name(&self) -> &str` | Planner identifier |
