...
4. After a parallel wave completes, results are **sorted and merged** back into the shared history
...
| Method | Description |
|--------|-------------|
| `get_ready_steps()` | Returns steps with all dependencies `Completed` |
| `mark_status(id, status)` | Update a step's status by ID |
| `pending_count()` | Count remaining `Pending` steps |
| `has_deadlock()` | Detect circular deps or all deps failed |
...
|-------|--------|-------------|
| `StepStart` | `step_id`, `description`, `step_number`, `total_steps` | A plan step began execution. During parallel waves, multiple `StepStart` events fire before any `StepEnd`. |
| `StepEnd` | `step_id`, `status`, `step_number`, `total_steps` | A plan step finished. `status` is `Completed` or `Failed`. |
| `GoalProgress` | `goal`, `progress`, `completed_steps`, `total_steps` | Fires after each wave completes. `progress` is 0.0–1.0. |
| `TaskUpdated` | `session_id`, `tasks` | Task list changed (added, status change, completed). Includes the full current task list. |
...
No changes to the planning config are needed — enable planning as usual, and configure lane handlers to route execution to external workers:
...
use a3s_code_core::{
    ...
    SessionLane, LaneHandlerConfig, TaskHandlerMode,
};
...
let (mut rx, _handle) = session.stream("Refactor auth, update tests, verify CI").await?;
while let Some(event) = rx.recv().await {
    match event {
...
// Route Execute lane to remote workers
await session.setLaneHandler('execute', { mode: 'external', timeoutMs: 120000 });

const events = await session.stream('Refactor auth, update tests, verify CI');
for (const event of events) {
  if (event.type === 'step_start')
    console.log(`[${event.stepNumber}/${event.totalSteps}] ${event.description}`);
...
# Route Execute lane to remote workers
session.set_lane_handler("execute", mode="external", timeout_ms=120000)

for event in session.stream("Refactor auth, update tests, verify CI"):
    if event.event_type == "step_start":
        print(f"[{event.step_number}/{event.total_steps}] {event.description}")
    elif event.event_type == "external_task_pending":
...
For the full external task handling API, handler modes, worker process examples, and dynamic lane switching, see [Lane Queue → External Task Handling](/docs/code/lane-queue#external-task-handling).
