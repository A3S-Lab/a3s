...
The Lane Queue system routes tool execution through a priority-based queue backed by [a3s-lane](/docs/lane). When enabled, read-only tools execute in parallel while write tools maintain sequential ordering.
...
| Query | P1 | read, glob, grep, ls, search, list_files | **Parallel** |
