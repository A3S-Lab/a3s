...
Parallelization is **opt-in** — by default all tools execute serially. When enabled, Query-lane tools (read, glob, grep, web_fetch, web_search, etc.) execute in parallel, providing significant speedup for network I/O operations.
...
// Optional: customize the parallelization strategy
...
session = agent.session("/project", options)
```
</Tab>
</Tabs>
...
|-------|------|---------|-------------|
| `min_tool_count` | usize | 8 | Minimum number of Query-lane tools in a single LLM turn to trigger parallelization |
| `allowed_tools` | Vec&lt;String&gt; | [] (all Query-lane) | Only parallelize these tools. Empty = allow all Query-lane tools |
| `blocked_tools` | Vec&lt;String&gt; | [] | Never parallelize these tools. Takes precedence over `allowed_tools` |

### Performance

When fetching 10 web pages:

| Mode | Tool Execution | Total (incl. LLM) |
|------|---------------|-------------------|
| Serial (default) | ~17s | ~42s |
| Parallel (opt-in) | ~1.8s | ~26s |
| **Speedup** | **~9x** | **~1.6x** |

<Callout type="info">
The total speedup is bounded by LLM response time. The tool execution speedup is dramatic (9x+) for network I/O, but LLM latency dominates the overall time.
</Callout>

### When to Use

- ✅ **Network I/O** — `web_fetch`, `web_search`, API calls (3-10x speedup)
- ✅ **Bulk file reads** — Many `read`/`grep` calls on large files
- ❌ **Fast local ops** — `glob`, `ls`, `list_files` (overhead > benefit)
- ❌ **Few tools** — Less than `min_tool_count` tools won't trigger parallelization
