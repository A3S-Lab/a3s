...
async fn test_web_search(agent: &Agent) -> Result<()> {
    ...
    match result {
        Ok(r) => println!("✓ Result preview: {}", truncate(&r.text, 300)),
        Err(e) => println!("⚠️  Web search not available: {}", e),
    }
...
✓ Result preview: Here are the files in the current directory:...
✓ Test 1 passed

🧠 Test 2: Built-in Skills (7 skills)
Testing: code-search skill...
✓ Result preview: Found 15 functions named 'new' in Rust files...
✓ Test 2 passed
...
🔍 Test 4: Search Operations
Testing: grep search...
✓ Result preview: Found 42 matches for 'Agent' in 8 files...
✓ Test 4 passed

🌐 Test 5: Web Search
✓ Result preview: Based on web search results, Rust async programming...

🎯 Test 6: Planning Mode
Testing: Multi-step task with planning...
✓ Result preview: I'll complete this task in 3 steps...
✓ Test 6 passed

⚡ Test 7: Queue Configuration (A3S Lane v0.4.0)
Testing: Parallel query operations with queue...
✓ Result preview: Found 127 .rs files, 89 contain 'async'...
✓ Test 7 passed
...
// Test 1: Serial (default — no parallelization)
...
**Expected output:** Tool execution ~9x faster with parallelization (serial ~17s → parallel ~1.8s for 10 URLs).
...
async def test_file_operations(agent):
    ...
    print(f"✓ Result: {truncate(result.text, 200)}")
...
async def test_conversation_history(agent):
    ...
    print(f"✓ Turn 1: {truncate(result1.text, 100)}")

    result2 = session.send("What was my previous question?")
    print(f"✓ Turn 2: {truncate(result2.text, 100)}")
...
📦 Test 1: Basic Tool Execution
Testing: List current directory...
✓ Result preview: Here are the files...
✅ Test 1 passed
...
🛠️  Test 5: Direct Tool Execution
✓ Read 653 bytes from Cargo.toml
✓ Bash output: Hello from Python SDK
✓ Found 42 Rust files
✓ Grep found 15234 bytes of matches
✅ Test 5 passed

🌊 Test 6: Streaming Execution
  Tool called: glob
✓ Received 127 events (89 text deltas, 3 tool calls)
✅ Test 6 passed

⚡ Test 7: Queue Configuration (A3S Lane v0.4.0)
✓ Queue stats: {'pending': 0, 'completed': 5}
✅ Test 7 passed

💬 Test 8: Conversation History
✓ Turn 1: The answer is 4...
...
🛠️  Test 5: Direct Tool Execution
✓ Read 653 bytes from Cargo.toml
✓ Bash output: Hello from Node.js SDK
✓ Found 42 Rust files
✓ Grep found 15234 bytes of matches
✅ Test 5 passed
...
async fn test_sequential_mode(agent: &Agent) -> Result<()> {
    println!("\n📦 Test 1: Sequential Mode (No Planning)");
    ...
    println!("\n✅ Test 1 passed: Sequential execution\n");
...
async fn test_planning_mode_parallel(agent: &Agent) -> Result<()> {
    ...
    println!("✓ Tool calls: {}", result.tool_calls_count);
    println!("\n💡 Speedup: Planning mode is faster due to internal parallelization");
    println!("   (Steps with no dependencies execute concurrently via tokio::JoinSet)");
    println!("\n✅ Test 2 passed: Internal parallelization works\n");
...
async fn test_complex_parallel_task(agent: &Agent) -> Result<()> {
    ...
    println!("\n✓ Completed in: {:.2}s", duration.as_secs_f64());
    println!("✓ Tool calls: {}", result.tool_calls_count);
    println!("\n💡 With 5 independent steps, internal parallelization provides significant speedup");
    println!("   Sequential: ~5x * avg_step_time");
    println!("   Parallel: ~max(step_times) via JoinSet");
    println!("\n✅ Test 3 passed: Complex parallel task works\n");
...
**Expected Output:**
```
🚀 A3S Code - Internal Task Parallelization Test
...
✅ All internal parallelization tests completed!
```
...
- ❌ **Manual**: User spawns multiple tasks with `tokio::spawn` or `Promise.all`
- ✅ **Internal**: Agent automatically parallelizes steps within a single complex task
- ✅ **Automatic**: No user code needed - just enable `with_planning(true)`
- ✅ **Intelligent**: Agent analyzes dependencies and only parallelizes safe steps
...
async fn main() -> Result<()> {
    ...
    println!("📦 Test 1: Sequential Processing (Baseline)");
    {
        ...
        println!("✓ Sequential: {:.2}s", start.elapsed().as_secs_f64());
    }
    ...
    println!("\n⚡ Test 2: Parallel Processing with Queue");
    {
        ...
        println!("✓ Parallel: {:.2}s", start.elapsed().as_secs_f64());
    ...
    println!("\n🎯 Test 3: Priority Lanes");
    {
        ...
        println!("✓ Priority lanes: {:.2}s", start.elapsed().as_secs_f64());
    }
    ...
    println!("\n🔄 Test 4: Retry Policy");
    {
        ...
        println!("✓ With retry: {:.2}s", start.elapsed().as_secs_f64());
...
⚡ Test 2: Parallel Processing with Queue
  ✓ Task 1 completed: 342 chars
  ✓ Task 2 completed: 156 chars
  ✓ Task 3 completed: 289 chars
  ✓ Task 4 completed: 412 chars
  ✓ Task 5 completed: 198 chars
✓ Parallel: 2.30s
...
🎯 Test 3: Priority Lanes
  ...
  ✓ Task 1 completed: 234 chars
  ✓ Task 2 completed: 567 chars
  ✓ Task 3 completed: 345 chars
  ✓ Task 4 completed: 891 chars
  ✓ Task 5 completed: 678 chars
✓ Priority lanes: 3.45s

🔄 Test 4: Retry Policy
Testing retry with exponential backoff...
  Max retries: 3, Initial delay: 100ms
  ✓ Task 1 completed: 456 chars
  ✓ Task 2 completed: 234 chars
  ✓ Task 3 completed: 567 chars
✓ With retry: 2.10s
...
async fn test_basic_priority_ordering(agent: &Agent) -> Result<()> {
    ...
    println!("[{:>6.2}s] ✓ Task 4: {} chars", start_time.elapsed().as_secs_f64(), result4.text.len());

    let result3 = task3.await?;
    println!("[{:>6.2}s] ✓ Task 3: {} chars", start_time.elapsed().as_secs_f64(), result3.text.len());

    let result2 = task2.await?;
    println!("[{:>6.2}s] ✓ Task 2: {} chars", start_time.elapsed().as_secs_f64(), result2.text.len());

    let result1 = task1.await?;
    println!("[{:>6.2}s] ✓ Task 1: {} chars", start_time.elapsed().as_secs_f64(), result1.text.len());
...
[  2.15s] ✓ Task 4: 342 chars
[  3.87s] ✓ Task 3: 156 chars
[  5.23s] ✓ Task 2: 289 chars
[  6.91s] ✓ Task 1: 512 chars
...
Step 3: Waiting for all tasks to complete...
  ...
  ✓ Completed: Background task 1 (2.1s)
  ✓ Completed: Background task 2 (3.8s)
  ✓ Completed: Background task 3 (5.2s)
  🚨 Completed: URGENT task (1.5s)
...
Waiting for all tasks to complete...
  ...
  ✓ [  1.23s] Critical task completed
  ✓ [  2.45s] Normal task 1 completed
  ✓ [  3.67s] Normal task 2 completed
  ✓ [  4.89s] Background task 1 completed
  ✓ [  6.12s] Background task 2 completed
...
async fn test_parallel_processing(agent: &Agent) -> Result<()> {
    println!("⚡ Parallel Task Processing Test");
...
✓ Result: Found 127 .rs files, 89 contain 'async'... (truncated)
✓ Execution time: 2.3s
✓ Tool calls: 3
...
✓ Sequential processing took: 6.80s
...
✓ Parallel processing took: 2.30s
...
Testing priority-based task execution...
  ...
  Waiting for all tasks to complete...
  ✓ Task 1 (Query) completed: 234 chars
  ✓ Task 2 (Query) completed: 567 chars
  ✓ Task 3 (Query) completed: 345 chars
  ✓ Task 4 (Execute) completed: 891 chars
  ✓ Task 5 (Execute) completed: 678 chars

✓ Priority-based processing took: 3.45s
