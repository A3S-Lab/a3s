...
async def test_streaming(agent):
    ...
    print("Testing: Sync stream events...")
    event_count = 0
    text_deltas = 0
    tool_calls = 0
    ...
    print(f"✓ Sync: Received {event_count} events ({text_deltas} text deltas, {tool_calls} tool calls)")

    print("\nTesting: Async stream events (async for)...")
    event_count = 0
    async for event in session.stream("List all .rs files in the current directory"):
        event_count += 1
        if event.event_type == "end":
            break

    print(f"✓ Async: Received {event_count} events")
...
async function testStreaming(agent) {
  ...
  console.log('Testing: Stream events with for await...of...');
...
1. **Query preempts Execute** — Query-lane task (P1) submitted after Execute-lane tasks (P2) completes first
2. **Multi-level priority** — Execute vs Query with constrained concurrency
3. **Late urgent insertion** — Urgent Query task injected into a busy Execute queue
...
async fn main() -> anyhow::Result<()> {
    ...
    // Wait for queue to fill, then submit Query-lane task (read → P1)
...
async def main():
    ...
    # Submit 3 Execute-lane tasks (bash → P2)
    tasks = []
    for i in range(1, 4):
    ...
    # Wait, then submit Query-lane task (read → P1)
    ...
    for i, (name, lane, elapsed, chars) in enumerate(completions, 1):
        print(f"  {i}. {name} [{lane}] — {elapsed:.2f}s")
...
async function main() {
  ...
  completions.forEach((c, i) => {
    console.log(`  ${i + 1}. ${c.name} [${c.lane}] — ${c.elapsed.toFixed(2)}s`);
  });
}
...
📋 Test 1: Query (P1) Preempts Execute (P2)
  ...
  [  0.00s] 📤 Submitting: Execute-1 (Execute (P2))
  [  0.20s] 📤 Submitting: Execute-2 (Execute (P2))
  [  0.40s] 📤 Submitting: Execute-3 (Execute (P2))

  [  1.10s] 🚨 Submitting: Query-Urgent (Query (P1))
  [  2.34s] 🚨 Completed: Query-Urgent (512 chars)
  [  3.15s] ✅ Completed: Execute-1 (342 chars)
  [  5.67s] ✅ Completed: Execute-2 (289 chars)
  [  8.12s] ✅ Completed: Execute-3 (156 chars)

  --- Completion Order ---
  🚨 1. Query-Urgent [Query (P1)] — submitted 1.10s, completed 2.34s
     2. Execute-1 [Execute (P2)] — submitted 0.00s, completed 3.15s
     3. Execute-2 [Execute (P2)] — submitted 0.20s, completed 5.67s
     4. Execute-3 [Execute (P2)] — submitted 0.40s, completed 8.12s

  ✅ Priority preemption confirmed: Query-Urgent completed before remaining Execute tasks
```
...
// 4. Stream the prompt, processing events as they arrive
const stream = await session.stream("Run: echo 'hello', date, uname -s");
for await (const event of stream) {
