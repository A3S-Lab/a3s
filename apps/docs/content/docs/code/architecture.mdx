...
A3S Code is a gRPC service (port 4088) that manages multiple AI coding sessions. Each session has its own context, permissions, and tool access. The server handles the full agentic loop — LLM calls, tool execution, confirmation, and streaming — so SDK clients only need to send prompts and receive events.
