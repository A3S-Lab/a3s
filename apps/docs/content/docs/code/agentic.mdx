...
```typescript
// Unary — wait for complete result
...
Stream events as the agent works — text output, tool calls, results, and more.
...
for await (const event of client.streamAgenticGenerate(sessionId, {
  ...
  switch (event.type) {
    ...
    case 'tool_call':
      ...
      break;
    case 'tool_result':
      console.log(`   → ${event.toolResult.output.slice(0, 200)}`);
      break;
    case 'step_finish':
    ...
    case 'plan':
      ...
      break;
    case 'reflection':
      console.log(`\n🤔 Confidence: ${event.confidence}, Retry: ${event.shouldRetry}`);
      break;
    case 'confirmation_required':
      console.log(`\n⚠️ Confirm: ${event.toolName}(${event.toolArgs})`);
      break;
    case 'context_compact':
      console.log(`\n📦 Compacted: ${event.beforeTokens} → ${event.afterTokens} tokens`);
      break;
    case 'done':
