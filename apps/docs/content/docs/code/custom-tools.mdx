...
> **Note:** In v0.7.2, custom tools are registered via skill files (`skill_dirs`) or MCP servers (`mcp_servers`), not via a Rust API on `CodeConfig`. Programmatic tool registration is planned for a future release.
