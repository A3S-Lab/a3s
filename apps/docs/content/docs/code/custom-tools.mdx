...
For embedding A3S Code as a library, register custom tools programmatically:
...
// Register via CodeConfig
let mut config = CodeConfig::from_file("agent.hcl")?;
config.extra_tools = vec![Arc::new(MyTool)];
...
| Rust API | `config.extra_tools` | Embedded library usage |
