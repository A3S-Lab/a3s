...
|----------|-----------|--------|
| AMD | SEV-SNP (Secure Encrypted Virtualization) | Primary support |
| Intel | SGX (Software Guard Extensions) | Supported |
| ARM | CCA (Confidential Compute Architecture) | Supported |
| Apple | Secure Enclave | Planned |
...
| TEE Available | Behavior |
|---------------|----------|
| Yes (SEV-SNP/SGX/CCA) | Full hardware memory encryption + attestation |
| No (VM only) | VM-level isolation via A3S Box (still secure, no hardware encryption) |
| Simulation mode | Fake TEE for development (`mock-tee` feature flag) |
