...
|----------|--------|-------------|
| `/api/v1/audit/events` | GET | List recent audit events |
| `/api/v1/audit/stats` | GET | Audit statistics summary |
| `/api/v1/audit/events/:session_id` | GET | Events for a specific session |

### Events Endpoints

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/v1/events` | GET | List system events |
| `/api/v1/events/:id` | GET | Get event by ID |
| `/api/v1/events/subscribe` | GET | SSE event stream |
...
| `/api/v1/gateway/status` | GET | Gateway status with session counts |
| `/.well-known/a3s-service.json` | GET | Service discovery |
