...
| Strategy | Distribution | Ordering | Use Case |
|----------|-------------|----------|----------|
| `None` | Single queue | Strict FIFO | Simple workloads |
| `RoundRobin` | Even spread | Per-partition FIFO | Balanced throughput |
| `Hash` | By command type | Per-type FIFO | Type-affinity (caching) |
| `Auto` | Per-core | Per-partition FIFO | Maximum parallelism |
...
|---------|---------------|
| Retry policies | Always (at least `exponential(2)`) |
| Dead letter queue | Always (inspect failures) |
| Persistent storage | When commands must survive restarts |
| Metrics | Production environments |
| Alerts | When SLA compliance matters |
| Rate limiting | External API calls, shared resources |
| Priority boosting | Mixed priority workloads |
| Partitioning | CPU-bound, high-throughput |
