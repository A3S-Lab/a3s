...
| `timeout_secs` | `Option<u64>` | Publish timeout in seconds |
...
| Field | Type | Description |
|-------|------|-------------|
| `deliver_policy` | `DeliverPolicy` | Where to start consuming (default: `All`) |
| `max_deliver` | `Option<i64>` | Max redelivery attempts before DLQ |
| `ack_wait_secs` | `Option<u64>` | Seconds to wait for ack before redelivery |
| `max_ack_pending` | `Option<i64>` | Backpressure: max unacked messages |
| `backoff_secs` | `Vec<u64>` | Redelivery backoff schedule in seconds |
...
|---------|---------|---------|
| `events.orders.>` | All subjects under `events.orders.` at any depth | `events.orders.created`, `events.orders.us.shipped` |
| `events.orders.*` | One level under `events.orders.` | `events.orders.created` but not `events.orders.us.shipped` |
...
| NatsProvider | Checks NATS connection via `info()` |
