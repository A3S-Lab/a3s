...
```
INFO event.publish{subject="orders.created" event_type="order.created" version=1}: a3s_event: publishing event
...
| NatsProvider | Checks NATS connection status |

Use in Kubernetes readiness probes or load balancer health checks:

```rust
async fn health_handler(bus: &EventBus) -> impl Reply {
