...
| Component | Purpose |
|-----------|---------|
| Gateway | Top-level orchestrator — ties configuration, listeners, routing, scaling, and observability into a single unit |
| Entrypoint | Named listeners on ports with optional TLS (HTTP, HTTPS, TCP, UDP) |
| Router | Request matching by Host, Path, PathPrefix, Headers, Method, and SNI |
| Middleware | Composable request/response transformations (15 built-in) |
| Service | Backend pool with load balancing, health checks, failover, and traffic mirroring |
| Provider | Dynamic configuration sources (file watcher with hot reload, DNS discovery, Kubernetes) |
| Proxy | Protocol-specific request forwarding (HTTP, WebSocket, gRPC, SSE, TCP, UDP) |
| Scaling | Knative-style autoscaler with concurrency limiting, request buffering, revision routing, and gradual rollouts |

## Protocol Support

| Protocol | Features |
|----------|----------|
| HTTP/1.1 & HTTP/2 | Full reverse proxy with hop-by-hop header filtering |
| WebSocket | Upgrade detection, bidirectional relay, multiplexing |
| SSE / Streaming | Chunked transfer for LLM outputs and event streams |
| gRPC | HTTP/2 h2c forwarding with header translation |
| TCP | Raw byte relay with SNI-based routing and IP filtering |
