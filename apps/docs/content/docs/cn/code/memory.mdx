...
<TypeTable
  type={
    "Episodic": {
      description: "用途: 具体事件和交互记录 · 默认重要性: 0.5",
    },
    "Semantic": {
      description: "用途: 事实和领域知识 · 默认重要性: 0.5–0.8",
    },
    "Procedural": {
      description: "用途: 成功模式（turn 完成后自动存储） · 默认重要性: 0.8",
    },
    "Working": {
      description: "用途: 当前任务的活跃上下文 · 默认重要性: 不固定",
    },
  }
/>
...
<TypeTable
  type={
    "id": {
      description: {`类型: \`String\` · 说明: UUID，自动生成`},
    },
    "content": {
      description: {`类型: \`String\` · 说明: 记忆内容`},
    },
    "memory_type": {
      description: {`类型: \`MemoryType\` · 说明: \`Episodic\`、\`Semantic\`、\`Procedural\`、\`Working\``},
    },
    "importance": {
      description: {`类型: \`f32\` · 说明: 0.0–1.0，自动截断`},
    },
    "tags": {
      description: {`类型: \`Vec<String>\` · 说明: 分类标签`},
    },
    "metadata": {
      description: {`类型: \`HashMap<String, String>\` · 说明: 任意键值对`},
    },
    "timestamp": {
      description: {`类型: \`DateTime<Utc>\` · 说明: 创建时间`},
    },
    "access_count": {
      description: {`类型: \`u32\` · 说明: 检索次数`},
    },
    "last_accessed": {
      description: {`类型: \`Option<DateTime<Utc>>\` · 说明: 最近检索时间`},
    },
  }
/>
...
<TypeTable
  type={
    "MemoryStored": {
      description: "记忆条目已持久化",
    },
    "MemoryRecalled": {
      description: "记忆条目已被检索用于上下文",
    },
    "MemoriesSearched": {
      description: "执行了一次记忆搜索",
    },
    "MemoryCleared": {
      description: "所有记忆已清除",
    },
...
<TypeTable
  type={
    ".with_file_memory(path)": {
      description: {`在指定路径启用 \`FileMemoryStore\``},
    },
    ".with_memory(Arc<dyn MemoryStore>)": {
      description: "使用自定义后端",
    },
    ".with_memory_config(MemoryConfig)": {
      description: "配置相关性评分和各层容量",
    },
  }
/>
...
<TypeTable
  type={
    "store(item)": {
      description: "持久化记忆条目（按 ID upsert）",
    },
    "retrieve(id)": {
      description: "按 ID 获取单条记忆",
    },
    "search(query, limit)": {
      description: "子串搜索，按相关性排序",
    },
    "search_by_tags(tags, limit)": {
      description: "按标签过滤，按相关性排序",
    },
    "get_recent(limit)": {
      description: "最近创建的记忆",
    },
    "get_important(threshold, limit)": {
      description: "重要性高于阈值的记忆，降序排列",
    },
    "delete(id)": {
      description: "删除单条记忆",
    },
