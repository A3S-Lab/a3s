...
| 字段 | 必填 | 描述 |
|-------|----------|-------------|
| `name` | 是 | 唯一的 skill 标识符（kebab-case） |
| `description` | 是 | skill 功能的简要描述 |
| `allowed-tools` | 否 | 逗号分隔的允许工具列表，支持通配符 |
| `kind` | 是 | Skill 类型：`instruction`、`tool` 或 `agent` |
| `tags` | 否 | 用于分类的标签列表 |
| `version` | 否 | 语义化版本（例如 "1.0.0"） |
