...
- ✅ **默认开启** — 7 个内置 skills 无需任何配置即自动加载
- ✅ **自动系统提示注入** — Skill 元数据（名称 + 描述）自动注入到系统提示中
- ✅ **全局 + 会话 skills 合并** — 会话级 skills 叠加在全局 skills 之上，互不干扰
- ✅ **工具权限控制** — Skills 限制可使用的工具
- ✅ **兼容 Claude Code 格式** — 可使用现有的 Claude Code skills
- ✅ **从目录加载** — 将 skills 组织在文件夹中，全局或按会话加载
- ✅ **编程式创建** — 在代码中创建 skills

## 内置 Skills（默认开启）

从 v0.9.2 起，7 个内置 skills **无需任何配置即自动加载**，其元数据（名称 + 描述）会自动注入到系统提示中。
...
所有内置工具的文档：read、write、edit、patch、bash、grep、glob、ls、web_fetch、web_search、git_worktree、batch、task、parallel_task、run_team。
...
**允许的工具：** `task(*), parallel_task(*), run_team(*)`
...
内置 skills 从 v0.9.2 起**默认自动加载**，无需任何配置。创建 session 时，7 个内置 skills 的元数据会自动注入到系统提示中。
...
<TypeTable
  type={{
    ...
    "kind": {
      description: "必填: 是 · 描述: Skill 类型：instruction 或 persona",
    },
    "tags": {
...
<Callout type="info">
**v1.3.4 新功能：** Skills 现在可以作为**一等工具**被调用，使用 `Skill` 工具，在执行期间临时授予权限。
</Callout>

### 3. Skill Tool - 可调用的 Skills 与权限隔离

`Skill` 工具允许 LLM 将 skills 作为可调用工具来调用，在执行期间临时授予 skill 的 `allowed-tools` 权限。这强制执行基于 skill 的访问模式，防止 agent 绕过 skills 直接访问底层工具。

#### 工作原理

```
1. LLM 调用: Skill("data-processor", prompt="Read README.md")
2. SkillTool 从 skill 的 allowed-tools 创建临时 PermissionPolicy
3. 创建新的 AgentLoop，使用 skill 的权限
4. Skill 使用临时权限执行
5. 执行完成后，权限自动撤销（RAII）
```
...
const session = agent.session('.', {
  ...
  permissionPolicy: new PermissionPolicy({
    ...
    deny: [new PermissionRule('read(*)')],    // 拒绝直接 read 访问
    defaultDecision: 'deny'
  })
});

// Agent 只能通过 skills 访问 read/grep
// 直接 read 调用会被拒绝
const result = await session.send('使用 data-processor 读取 file.txt');
```
</Tab>
<Tab value="Python">
```python
...
session = agent.session(".", SessionOptions(
    ...
    permission_policy=PermissionPolicy(
        ...
        deny=[PermissionRule("read(*)")],    # 拒绝直接 read 访问
        default_decision="deny"
    )
))

# Agent 只能通过 skills 访问 read/grep
result = session.send("使用 data-processor 读取 file.txt")
```
</Tab>
</Tabs>
...
1. **权限隔离**：父 agent 无法绕过 skills 访问底层工具
2. **临时授权**：权限仅在 skill 执行期间授予
3. **自动撤销**：RAII 模式确保权限被清理
4. **可组合安全**：与权限策略结合实现细粒度控制

#### 使用场景

- **受限环境**：允许 agent 仅通过批准的 skills 使用强大工具
- **审计追踪**：跟踪调用了哪些 skills 以及它们使用了哪些工具
- **Skill 组合**：通过链式调用 skill 构建复杂工作流
- **安全边界**：在 skill 级别强制执行安全策略
...
**使用场景：** 赋予 agent 特定角色（如"安全专家"、"技术写作者"）
