...
`storage_url` 会被解析为自定义存储元数据，但它不是本地文件型 session
persistence 路径。ACL 中使用 `sessions_dir`，或在 SDK options 中传
`sessionStore`。
...
当前 SDK surface 没有 `session.btw()` helper。需要不改变 session transcript
的临时问题时，使用显式 history：
