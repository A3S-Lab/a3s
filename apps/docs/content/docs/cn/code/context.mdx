...
| 事件 | 触发时机 |
|-------|------|
| `ContextWarning` | 上下文使用量超过阈值 |
| `ContextCompacting` | 压缩开始 |
| `ContextCompacted` | 压缩完成 |
...
| 字段 | 类型 | 描述 |
|-------|------|-------------|
| `query` | `String` | 搜索查询（通常是用户的提示） |
| `context_types` | `Vec<ContextType>` | 要检索的类型 |
| `depth` | `ContextDepth` | 返回多少细节 |
| `max_results` | `usize` | 最大返回条目数 |
| `max_tokens` | `usize` | 结果的 token 预算 |
| `session_id` | `Option<String>` | 将结果限定到某个会话 |
| `params` | `HashMap<String, String>` | 提供者特定的参数 |
...
| 类型 | 描述 |
|------|-------------|
| `Memory` | 记忆条目（episodic、semantic、procedural、working） |
| `Resource` | 外部资源（文件、文档、代码）— 默认 |

#### ContextDepth

| 深度 | Token 预算 | 描述 |
|-------|-------------|-------------|
| `Abstract` | ~100 tokens | 简要摘要 |
| `Overview` | ~2,000 tokens | 适度细节（默认） |
| `Full` | 可变 | 完整内容 |
...
|-------|------|-------------|
| `id` | `String` | 唯一条目标识符 |
| `context_type` | `ContextType` | Memory 或 Resource |
| `content` | `String` | 上下文内容 |
| `token_count` | `usize` | 估计的 token 数 |
| `relevance` | `f32` | 0.0–1.0 相关性分数 |
| `source` | `Option<String>` | 上下文来源 |
...
| 事件 | 触发时机 |
|-------|------|
| `ContextResolving` | 上下文提供者被查询 |
| `ContextResolved` | 上下文条目已检索 |
| `ContextWarning` | 上下文使用量超过阈值 |
| `ContextCompacting` | 压缩开始 |
| `ContextCompacted` | 压缩完成 |
...
|--------|------|--------|---------|---------|
| 文件系统上下文 | `.with_fs_context(path)` | `fs_context=path` | `fsContext: path` | `None` |
| 自定义提供者 | `.with_context_provider(Arc::new(p))` | _（基于配置）_ | _（基于配置）_ | `None` |
| 压缩阈值 | `.with_context_threshold(f)` | `context_threshold=f` | `contextThreshold: f` | `0.8` |

### ContextProvider trait（Rust）

| 方法 | 签名 | 描述 |
|--------|-----------|-------------|
| `retrieve` | `async fn retrieve(&self, query: &ContextQuery) -> Result<Vec<ContextItem>>` | 获取相关上下文条目 |
| `on_turn_complete` | `async fn on_turn_complete(&self, turn: &TurnSummary) -> Result<()>` | 每个 agent 轮次后调用 |
| `name` | `fn name(&self) -> &str` | 提供者标识符 |
...
|-------|------|-------------|
| `query` | `String` | 自然语言查询 |
| `max_tokens` | `Option<usize>` | 结果的 token 预算 |
| `context_types` | `Vec<ContextType>` | 按类型过滤 |
| `depth` | `ContextDepth` | `Abstract` 或 `Full` |

### ContextItem 字段

| 字段 | 类型 | 描述 |
|-------|------|-------------|
| `content` | `String` | 上下文文本 |
| `source` | `String` | 来源（文件路径、记忆 ID 等） |
...
| 方法 | 描述 | 默认值 |
|--------|-------------|---------|
| `VectorContextConfig::new(path)` | 设置工作区根目录 | — |
| `.with_min_relevance(f)` | 最小余弦相似度 | `0.3` |
| `.with_max_results(n)` | 每次查询最大块数 | `10` |
| `.with_chunk_size(n)` | 每块最大字符数 | `1500` |
