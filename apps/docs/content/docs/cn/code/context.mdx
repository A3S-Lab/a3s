...
const session = agent.session('/project', {
  autoCompact: true,
  autoCompactThreshold: 0.8,  // 在 80% 时压缩（默认）
});
```
...
session = agent.session('/project', {
    'auto_compact': True,
    'auto_compact_threshold': 0.8,  # 在 80% 时压缩（默认）
})
```
...
<Callout type="info">
  `MemoryContextProvider` 在 Rust 中实现。TypeScript 和 Python 会话可以通过传递 `FileMemoryStore` 实例来启用内存支持的上下文 — 提供者将自动激活。
</Callout>
...
const agent = await Agent.create('agent.hcl');
const session = agent.session('/project', {
  memoryStore: new FileMemoryStore('./.a3s/memory'),  // 自动启用 MemoryContextProvider
});
```
...
agent = Agent.create('agent.hcl')
session = agent.session('/project',
    memory_store=FileMemoryStore('./.a3s/memory'))  # 自动启用 MemoryContextProvider
```
...
自定义上下文提供者在 Rust 中实现 `ContextProvider` trait 并在会话上注册。TypeScript 和 Python 可以使用 `fsContext` 注入文件系统上下文，无需自定义提供者。
...
对于完整的自定义提供者逻辑（语义搜索、外部知识库），请在 Rust 中实现 `ContextProvider` 并通过 `.with_context_provider()` 注册。
...
const agent = await Agent.create('agent.hcl');
const session = agent.session('/project', {
  memoryStore: new FileMemoryStore('./.a3s/memory'),  // 记忆提供者
  fsContext: './docs',          // 文件系统上下文提供者
});
```

多个内置提供者可以通过会话选项组合。对于完全自定义的提供者，请使用 Rust 并通过 `.with_context_provider()` 注册。
...
agent = Agent.create('agent.hcl')
session = agent.session('/project',
    memory_store=FileMemoryStore('./.a3s/memory'),  # 记忆提供者
    fs_context='./docs')  # 文件系统上下文提供者
```
...
<TypeTable
  type={{
    ...
    "自定义提供者": {
      description: "Python: _（基于配置）_ · TypeScript: _（基于配置）_ · 默认值: None",
    },
    "压缩阈值": {
      description: "Python: context_threshold=f · TypeScript: contextThreshold: f · 默认值: 0.8",
    },
  }}
/>
...
<TypeTable
  type={{
    "RipgrepContextConfig::new(path)": {
      description: "描述: 设置工作区根目录",
    },
    ".with_case_insensitive(b)": {
      description: "描述: 大小写不敏感搜索 · 默认值: true",
    },
    ".with_context_lines(n)": {
      description: "描述: 匹配行前后的上下文行数 · 默认值: 2",
    },
    ".with_max_file_size(n)": {
      description: "描述: 最大文件大小 · 默认值: 1MB",
    },
  }}
/>
