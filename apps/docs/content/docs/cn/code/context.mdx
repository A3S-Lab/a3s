...
<Callout type="info">
  `MemoryContextProvider` 在 Rust 中实现。TypeScript 和 Python 会话可以通过在会话选项中设置 `memoryDir` 来启用内存支持的上下文 — 提供者将自动激活。
</Callout>
...
const agent = await Agent.create('agent.hcl');
const session = agent.session('/project', {
  memoryDir: './.a3s/memory',  // 自动启用 MemoryContextProvider
});
```
...
agent = Agent.create('agent.hcl')
session = agent.session('/project', {
    'memory_dir': './.a3s/memory',  # 自动启用 MemoryContextProvider
})
```
...
<TypeTable
  type={{
    ...
    "自定义提供者": {
      description: "Rust: .with_context_provider(Arc::new(p)) · Python: _（基于配置）_ · TypeScript: _（基于配置）_ · 默认值: None",
    },
    "压缩阈值": {
      description: "Rust: .with_context_threshold(f) · Python: context_threshold=f · TypeScript: contextThreshold: f · 默认值: 0.8",
    },
  }}
/>
