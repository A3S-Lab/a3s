...
A3S Code 提供 15 个内置工具，支持自定义工具注册和 MCP 集成。所有工具通过 `ToolExecutor` 统一管理。

<Callout type="info">
内置工具：15 个 | 自定义工具：通过 `Tool` trait | MCP 工具：通过 `mcp_servers` 配置
</Callout>
...
<TypeTable
  type={{
    "web_fetch": {
      description: "描述: 获取 URL 内容并转为 Markdown/文本（5MB 限制） · 参数: url, format?, timeout?",
    },
    "web_search": {
      description: "描述: 多引擎聚合搜索（DuckDuckGo、Brave、Wikipedia 等） · 参数: query, engines?, limit?, timeout?, proxy?, format?",
    },
  }}
/>
...
<TypeTable
  type={{
    ...
    "parallel_task": {
      description: "描述: 并发委托多个子 Agent · 参数: tasks[]",
    },
    "run_team": {
      description: "描述: Lead → Worker → Reviewer 团队工作流 · 参数: goal, lead_agent?, worker_agent?, reviewer_agent?, max_steps?",
    },
  }}
/>
