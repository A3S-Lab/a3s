...
| 方法 | 描述 |
|--------|-------------|
| `session_id()` | 返回会话的唯一 ID |
| `save()` | 将当前历史持久化到存储（无存储时为空操作） |
| `agent.resume_session(id, opts)` | 加载已保存的会话并恢复历史 |
...
| `id` | 会话标识符 |
| `messages` | 完整对话历史 |
| `config.workspace` | 工作区路径 |
| `config.system_prompt` | 系统提示 |
| `config.planning_enabled` | 规划标志 |
| `tool_names` | 工具名称（恢复时重建） |
...
|--------|------|--------|---------|---------|
| 自定义存储 | `.with_session_store(Arc::new(s))` | _（仅 Rust）_ | _（仅 Rust）_ | `FileSessionStore` |

### SessionStore trait（Rust）

| 方法 | 签名 | 描述 |
|--------|-----------|-------------|
| `save` | `async fn save(&self, data: &SessionData) -> Result<()>` | 持久化会话数据 |
| `load` | `async fn load(&self, id: &str) -> Result<Option<SessionData>>` | 按 ID 加载会话 |
| `delete` | `async fn delete(&self, id: &str) -> Result<()>` | 删除会话 |
| `list` | `async fn list(&self) -> Result<Vec<String>>` | 列出所有会话 ID |
...
| `tasks` | `Vec<AgentTask>` | 任务列表（别名：`todos`） |
| `config.workspace` | `String` | 工作区路径 |
| `config.system_prompt` | `String` | 系统提示 |
| `config.planning_enabled` | `bool` | 规划标志 |
| `tool_names` | `Vec<String>` | 工具名称（恢复时重建） |
| `created_at` | `DateTime<Utc>` | 会话创建时间 |
| `updated_at` | `DateTime<Utc>` | 最后保存时间 |

### 会话管理方法

| 方法 | Rust | 描述 |
|--------|------|-------------|
| 保存会话 | `session.save().await?` | 持久化当前状态 |
| 加载会话 | `agent.resume(id, options?)?` | 从存储状态恢复 |
| 列出会话 | `agent.list_sessions().await?` | 列出所有已保存的会话 ID |
| 删除会话 | `agent.delete_session(id).await?` | 删除已存储的会话 |
