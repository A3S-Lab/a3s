...
<TypeTable
  type={{
    ...
    "system_prompt": {
      description: "类型: string · 必填: ❌ · 描述: Agent 的系统提示/指令（旧版 — 映射到 extra 插槽）",
    },
    "prompt": {
      description: "类型: string · 必填: ❌ · 描述: Agent 的提示/指令",
    },
    "model": {
...
<TypeTable
  type={{
    "Agent 目录": {
      description: "Rust: .with_agent_dir(path) · Python: agent_dirs=[path] · TypeScript: agentDirs: [path] · 默认值: []",
    },
  }}
/>
...
<TypeTable
  type={{
    "explore": {
      description: "快速代码库探索（只读） · 最大步骤: 20 · 工具: read, grep, glob, ls",
    },
    "general": {
      description: "通用多步骤执行 · 最大步骤: 50 · 工具: 完整访问权限",
    },
    "plan": {
      description: "架构和规划（只读） · 最大步骤: 30 · 工具: read, grep, glob",
    },
  }}
/>
