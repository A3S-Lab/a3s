...
| Agent | 描述 | 最大步数 | 权限 |
|-------|-------------|-----------|-------------|
| `explore` | 快速代码库探索（只读） | 20 | read, grep, glob, ls |
| `general` | 通用多步骤执行 | 50 | 完整工具访问 |
| `plan` | 架构和规划（只读） | 30 | read, grep, glob |
| `title` | 会话标题生成（隐藏） | 1 | 无 |
| `summary` | 会话摘要（隐藏） | 5 | read |
...
| 字段 | 类型 | 必填 | 描述 |
|-------|------|----------|-------------|
| `name` | string | 是 | 唯一的 agent 标识符 |
| `description` | string | 是 | agent 的功能（展示给 LLM） |
| `mode` | `subagent` 或 `primary` | 否 | Agent 模式（默认：`subagent`） |
| `hidden` | bool | 否 | 从列表中隐藏（默认：`false`） |
| `max_steps` | number | 否 | 最大工具轮数 |
| `can_spawn_subagents` | bool | 否 | 允许递归委派（默认：`false`） |
| `permissions.allow` | string[] | 否 | 允许的工具名称 |
| `permissions.deny` | string[] | 否 | 拒绝的工具名称 |
| `model` | string | 否 | 模型覆盖，`provider/model` 格式 |
| `prompt` | string | 否 | Agent 的系统提示 |
...
| 字段 | 类型 | 必填 | 描述 |
|-------|------|----------|-------------|
| `agent` | string | 是 | Agent 名称（内置或自定义） |
| `description` | string | 是 | 简短任务描述 |
| `prompt` | string | 是 | 给子代理的详细提示 |
| `background` | bool | 否 | 不等待结果运行（默认：`false`） |
| `max_steps` | int | 否 | 覆盖 agent 的默认最大步数 |

### `task` 结果

| 字段 | 类型 | 描述 |
|-------|------|-------------|
| `output` | string | 子代理响应文本 |
| `session_id` | string | 子会话 ID |
...
| 字段 | 类型 | 必填 | 描述 |
|-------|------|----------|-------------|
| `name` | `string` | ✅ | Agent 标识符（用于 `task` 工具） |
| `description` | `string` | ✅ | Agent 的功能 |
| `system_prompt` | `string` | ✅ | Agent 的系统提示/指令 |
| `model` | `string` | ❌ | 模型覆盖（`provider/model` 格式） |
| `allowed_tools` | `string[]` | ❌ | 工具访问限制 |
| `max_tool_rounds` | `number` | ❌ | 每次调用的最大工具轮数 |

### SessionOptions

| 选项 | Rust | Python | Node.js | 默认值 |
|--------|------|--------|---------|---------|
| Agent 目录 | `.with_agent_dir(path)` | `agent_dirs=[path]` | `agentDirs: [path]` | `[]` |

### task 工具 schema

| 参数 | 类型 | 必填 | 描述 |
|-----|------|----------|-------------|
| `agent` | `string` | ✅ | 要委派的 agent 名称 |
| `prompt` | `string` | ✅ | 任务提示 |
| `workspace` | `string` | ❌ | 工作区覆盖 |

### 内置 agents

| Agent | 描述 |
|-------|-------------|
| `code-reviewer` | 审查代码的风格、安全性和正确性 |
| `test-writer` | 生成单元测试和集成测试 |
| `doc-writer` | 从代码编写文档 |
| `refactorer` | 重构代码以提高清晰度和性能 |
| `debugger` | 诊断和修复 bug |
