...
[`a3s` CLI](https://github.com/A3S-Lab/a3s) 提供的交互式终端应用；它驱动
...
| `a3s code` TUI | 运行现成的终端编程 agent。 | [A3S-Lab/a3s](https://github.com/A3S-Lab/a3s) |
...
| 定时服务 | [AgentDir](/cn/docs/code/agent-dir) 可通过 `serveAgentDir` / `serve_agent_dir` 运行定时 turn，每个启用 schedule 对应一个稳定的 `schedule:<name>` session。 |
