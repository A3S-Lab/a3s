...
| 内置 OCR 路由 | `mcp__use_ocr__*` | 面向对象安全的 `OcrProvider` 接口。A3S release 默认注入本地 `PP-OCRv6_small` ONNX provider；其他 provider 可声明不同的模型、可用性及数据是否离开设备，MCP annotations 会据此反映真实行为。 |
