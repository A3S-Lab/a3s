...
<TypeTable
  type={{
    ...
    "ToolExecutor": {
      description: "工具注册和执行。管理 16 个内置工具。",
    },
    "LlmPlanner": {
...
<TypeTable
  type={{
    "Control (P0)": {
      description: "用途: 最高优先级 — pause、resume、cancel 信号 · 默认模式: 内部（顺序）",
    },
    "Query (P1)": {
      description: "用途: 只读操作（read、grep、glob、ls、web_fetch、web_search） · 默认模式: 内部（并行）",
    },
    "Execute (P2)": {
...
在 11 个生命周期事件注册自定义逻辑：`PreToolUse`、`PostToolUse`、`GenerateStart`、`GenerateEnd`、`SessionStart`、`SessionEnd`、`SkillLoad`、`SkillUnload`、`PrePrompt`、`PostResponse`、`OnError`。参见 [Hooks](/cn/docs/code/hooks)。
</Accordion>
<Accordion title="子代理">
