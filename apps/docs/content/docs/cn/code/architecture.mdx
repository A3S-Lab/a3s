...
| 概念 | 描述 |
|---------|-------------|
| **Agent** | 配置驱动的顶层对象。持有 LLM 客户端和配置。与工作区无关。 |
| **AgentSession** | 绑定到特定工作区。所有 LLM 交互和工具执行都在此进行。 |
| **SessionOptions** | 可选的每会话覆盖：模型（`provider/model` 格式）、规划、HITL 等。 |
| **AgentLoop** | 核心执行引擎，驱动 LLM ↔ 工具的多轮循环。 |
| **ToolExecutor** | 工具注册和执行。管理 11 个内置工具。 |
| **LlmPlanner** | JSON 结构化规划 — 将复杂任务分解为带依赖图的执行计划。 |
| **WaveScheduler** | 将独立的计划步骤分组为 wave，通过 `JoinSet` 并行执行。 |
| **AgentEvent** | `#[non_exhaustive]` 事件枚举，在流式传输期间发出 — 安全支持 SDK 演进。 |
...
| 层 | 组件 | 功能 |
|-------|-----------|-------------|
| 1 | 权限策略 | Deny → Allow → Ask → Default 规则评估，支持 glob 模式 |
| 2 | HITL 确认 | 独立于权限 — 即使 `Allow` 也会经过 HITL 检查 |
| 3 | SecurityProvider | 可插拔 trait，用于污点追踪、输出清洗、注入检测 |
| 4 | 工作区边界 | 所有文件操作限制在会话工作区内 |
| 5 | HookEngine | 通过生命周期 hooks 实现自定义安全逻辑 |
...
| Lane | 用途 | 默认模式 |
|------|---------|--------------|
| Query | 只读操作（read、grep、glob、ls） | 内部（并行） |
| Execute | 写操作（write、edit、patch、bash） | 内部（顺序） |
| Generate | LLM 调用 | 内部 |
...
| 类型 | 用途 | 存储 |
|------|---------|---------|
| Episodic | 会话事件和交互 | FileMemoryStore（默认） |
| Semantic | 事实和知识 | FileMemoryStore（默认） |
| Procedural | 成功的模式和工作流 | FileMemoryStore（默认） |
| Working | 当前任务上下文 | 内存（短期） |
