...
| 模式 | 描述 | 使用场景 |
|------|-------------|----------|
| **Internal** | 任务在 agent 进程中执行（默认） | 单机部署，低延迟 |
| **External** | 任务发送到外部 worker，等待完成 | 多机器并行化，资源隔离 |
| **Hybrid** | 内部执行 + 通知外部系统 | 内部执行加外部监控/日志 |
...
| Lane | 优先级 | 工具 | 最适合 |
|------|----------|-------|----------|
| **Control** | P0（最高） | pause, resume, cancel | 控制操作 |
| **Query** | P1 | read, grep, glob, ls | 可并行化的读操作 |
| **Execute** | P2 | bash, write, edit | 写操作、命令 |
| **Generate** | P3（最低） | LLM 调用 | AI 生成 |
...
| `timeout_ms` | `Option<u64>` | 等待外部 worker 响应的最大时间 |

### 外部任务方法

| 方法 | Rust | Python | Node.js |
|--------|------|--------|---------|
| 设置处理器 | `session.set_external_task_handler(fn).await` | `session.set_external_task_handler(fn)` | `session.setExternalTaskHandler(fn)` |
| 轮询待处理 | `session.pending_external_tasks().await` | `await session.pending_external_tasks()` | `await session.pendingExternalTasks()` |
| 完成任务 | `session.complete_external_task(id, result).await` | `await session.complete_external_task(id, result)` | `await session.completeExternalTask(id, result)` |
...
| `payload` | `Value` | 任务数据（提示、工具参数等） |
| `created_at` | `DateTime<Utc>` | 入队时间戳 |
| `timeout_ms` | `Option<u64>` | 每任务超时覆盖 |
...
| `result` | `Option<Value>` | 任务输出载荷 |
| `error` | `Option<String>` | `success` 为 `false` 时的错误消息 |
| `duration_ms` | `Option<u64>` | Worker 执行时间 |
