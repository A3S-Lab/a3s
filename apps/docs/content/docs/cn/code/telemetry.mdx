...
遥测模块发出标准 OpenTelemetry span。每次 `send()` / `stream()` 调用期间 span 自动触发 — 无需修改 agent 代码。

<Tabs items={['TypeScript', 'Python', 'Rust']}>
<Tab value="TypeScript">

遥测自动发出。在启动进程之前通过环境变量配置收集器：

```typescript
import { Agent } from '@a3s-lab/code';

// 运行前设置环境变量（或在 shell 中设置）：
...
// Span 自动发出 — 无需额外代码
const result = await session.send('分析此代码库');
```
...
# Span 自动发出 — 无需额外代码
result = await session.send('分析此代码库')
```
...
通过环境变量配置 OTLP 收集器端点 — 应用程序代码中无需设置订阅者：
