...
`session.tool(name, args)`（以及 `glob`、`grep`、`readFile` 等类型化辅助方法）会直接运行宿主工具，循环中不发起任何模型调用。它们适用于测试、迁移以及宿主驱动的工作流——你需要确定性结果，而不是一次智能体回合。对于无人值守调用，优先使用显式 allow-list 和关闭式兜底，避免示例后续扩展成任意工具访问。
