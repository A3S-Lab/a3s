...
| 自定义上下文提供者 | `.with_context_provider(Arc::new(p))` | _（基于配置）_ | _（基于配置）_ | `None` |
...
| `.with_min_relevance(f)` | 最小余弦相似度分数 | `0.3` |
| `.with_max_results(n)` | 每次查询返回的最大分块数 | `10` |
| `.with_chunk_size(n)` | 每个分块的最大字符数 | `1500` |
