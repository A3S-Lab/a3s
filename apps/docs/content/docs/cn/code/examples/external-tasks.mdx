...
<TypeTable
  type={{
    "设置处理器": {
      description: "Rust: session.set_external_task_handler(fn).await · Python: session.set_external_task_handler(fn) · TypeScript: session.setExternalTaskHandler(fn)",
    },
    "轮询待处理": {
      description: "Rust: session.pending_external_tasks().await · Python: session.pending_external_tasks() · TypeScript: await session.pendingExternalTasks()",
    },
    "完成任务": {
      description: "Rust: session.complete_external_task(id, result).await · Python: session.complete_external_task(id, result) · TypeScript: await session.completeExternalTask(id, result)",
    },
  }}
/>
