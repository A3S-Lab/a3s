...
| 字段 | 类型 | 必填 | 描述 |
|-------|------|----------|-------------|
| `invocations` | `array` | ✅ | 要并行执行的工具调用列表 |
| `invocations[].tool` | `string` | ✅ | 要调用的工具名称 |
| `invocations[].args` | `object` | ✅ | 传递给工具的参数 |

### 直接调用

| 方法 | Rust | Python | Node.js |
|--------|------|--------|---------|
| 直接调用工具 | `session.call_tool("batch", json!({...})).await?` | `await session.call_tool("batch", {...})` | `await session.callTool('batch', {...})` |
