...
|--------|------|--------|---------|---------|
| 启用自动压缩 | `.with_auto_compact(true)` | `auto_compact=True` | `autoCompact: true` | `false` |
| 阈值（0.0–1.0） | `.with_auto_compact_threshold(f)` | `auto_compact_threshold=f` | `autoCompactThreshold: f` | `0.8` |

### ContextCompacted 事件

| 字段 | Rust | Python/Node.js | 描述 |
|-------|------|----------------|-------------|
| 压缩前消息数 | `messages_before` | `messages_before` / `messagesBefore` | 压缩前的消息数量 |
| 压缩后消息数 | `messages_after` | `messages_after` / `messagesAfter` | 压缩后的消息数量 |
| 节省的 token | `tokens_saved` | `tokens_saved` / `tokensSaved` | 大约释放的 token 数 |
