...
**运行：** `cargo run --example 08_hooks`
**源码：** [`core/examples/08_hooks.rs`](https://github.com/A3S-Lab/Code/blob/main/core/examples/08_hooks.rs)
</Tab>
<Tab value="Python">
```python
tool_log = []
...
<Tabs groupId="lang" items={['Python', 'TypeScript', 'Rust']}>
<Tab value="Python">
```python
...
impl HookHandler for SentinelGuard {
    fn handle(&self, event: &HookEvent) -> HookResponse {
        ...
        let cmd = e.args.to_string().to_lowercase();
        for pat in BLOCKED {
            if cmd.contains(&pat.to_lowercase()) {
                eprintln!("⛔  已阻断: '{pat}'");
                return HookResponse::block(format!("[哨兵] '{pat}'"));
            }
        }
        HookResponse::continue_()
    }
...
let result = session.send("用子 agent 分析代码仓库", None).await?;
println!("{}", result.text);
```
</Tab>
</Tabs>
