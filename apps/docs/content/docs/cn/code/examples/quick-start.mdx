...
|--------|------|--------|---------|---------|
| 宽松策略 | `.with_permissive_policy()` | `permissive=True` | `permissive: true` | `false` |
| 解析重试 | `.with_parse_retries(n)` | `max_parse_retries=n` | `maxParseRetries: n` | `3` |
| 工具超时 | `.with_tool_timeout(ms)` | `tool_timeout_ms=ms` | `toolTimeoutMs: ms` | `30000` |
| 熔断器 | `.with_circuit_breaker(n)` | `circuit_breaker_threshold=n` | `circuitBreakerThreshold: n` | `5` |

### AgentResponse

| 字段 | Rust | Python | Node.js |
|-------|------|--------|---------|
| 最终文本 | `result.text` | `result.text` | `result.text` |
| 总 token 数 | `result.usage.total_tokens` | `result.usage.total_tokens` | `result.usage.totalTokens` |
| 工具调用次数 | `result.tool_calls_count` | `result.tool_calls_count` | `result.toolCallsCount` |
| 历史记录 | `session.history()` | `session.history()` | `session.history()` |
