...
Rust session 构建以异步为先，因为默认 memory、文件型 store、queue、trajectory
recording 与 MCP discovery 都可能需要 I/O：
...
即使公开 stream handle 被丢弃或中止，runtime 也会在 producer 真正停止前继续
持有 lease。直接宿主工具 helper 不改变 transcript，因此不占用这把 lease。

Node 与 Python 的 stream iterator 会在 terminal boundary 等待这段 lifecycle
清理。iterator 完整消费并报告结束后，立即开始下一次对话操作不会继承上一个
stream 留下的过期 busy 状态。
...
`session.resumeRun(runId)` 恢复。参见[持久化](/cn/docs/code/persistence)。
