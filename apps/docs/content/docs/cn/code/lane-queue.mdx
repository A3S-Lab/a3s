...
<TypeTable
  type={{
    "提交任务": {
      description: "Python: await session.submit_task({...}) · TypeScript: await session.submitTask({...})",
    },
    "队列统计": {
      description: "Python: await session.queue_stats() · TypeScript: await session.queueStats()",
    },
    "队列指标": {
      description: "Python: await session.queue_metrics() · TypeScript: await session.queueMetrics()",
    },
    "死信": {
      description: "Python: await session.dead_letters() · TypeScript: await session.deadLetters()",
    },
    "设置 lane 处理器": {
      description: "Python: await session.set_lane_handler(lane, cfg) · TypeScript: await session.setLaneHandler(lane, cfg)",
    },
  }}
/>
