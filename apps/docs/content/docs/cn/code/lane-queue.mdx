...
使用 `submit()` 和 `submit_batch()` 从 SDK 代码直接向 lane 队列注入任务 — 绕过 LLM 轮次。适用于主动调度工作、扇出模式或从外部编排器驱动队列。
...
| Lane | 字符串值 | 优先级 |
|------|---------|--------|
| Control | `"control"` | P0（最高）|
| Query | `"query"` | P1 |
| Execute | `"execute"` | P2 |
| Generate | `"generate"` | P3（最低）|

`submit_batch()` 将所有 payload 提交到同一个 lane。Query lane 的批量任务并行执行（最多 `query_max_concurrency` 个）。其他 lane 顺序执行。
...
<TypeTable
  type={{
    "提交任务": {
      description: "Rust: session.submit_task(task).await? · Python: await session.submit_task({...}) · TypeScript: await session.submitTask({...})",
    },
    "队列统计": {
      description: "Rust: session.queue_stats().await · Python: await session.queue_stats() · TypeScript: await session.queueStats()",
    },
    "队列指标": {
      description: "Rust: session.queue_metrics().await · Python: await session.queue_metrics() · TypeScript: await session.queueMetrics()",
    },
    "死信": {
      description: "Rust: session.dead_letters().await · Python: await session.dead_letters() · TypeScript: await session.deadLetters()",
    },
    "设置 lane 处理器": {
      description: "Rust: session.set_lane_handler(lane, cfg).await · Python: await session.set_lane_handler(lane, cfg) · TypeScript: await session.setLaneHandler(lane, cfg)",
    },
  }}
/>
