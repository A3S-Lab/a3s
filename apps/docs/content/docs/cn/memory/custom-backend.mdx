...
impl MemoryStore for MyStore {
    ...
    async fn prune(&self, policy: &PrunePolicy) -> anyhow::Result<usize> { todo!() }
}
```
...
<TypeTable
  type={{
    "store(item)": {
      description: "持久化记忆（按 ID upsert）",
    },
    "retrieve(id)": {
      description: "按 ID 获取单条记忆",
    },
    "search(query, limit)": {
      description: "按内容搜索，按相关性排序",
    },
    ...
    "prune(policy)": {
      description: "可选 retention pass；默认 trait implementation 是 no-op",
    },
  }}
/>
...
impl MemoryStore for VectorStore {
    ...
    # async fn clear(&self) -> anyhow::Result<()> { todo!() }
    # async fn count(&self) -> anyhow::Result<usize> { todo!() }
    # async fn prune(&self, policy: &a3s_memory::PrunePolicy) -> anyhow::Result<usize> { let _ = policy; Ok(0) }
}
```

## 与 A3S Code 配合使用

A3S Code v2.1.0 Node SDK 支持 `FileMemoryStore` 和 `MemorySessionStore`。把任意
Rust `MemoryStore` 实现传入 A3S Code session 不属于本页范围。

本页用于说明 standalone `a3s-memory` backend 设计。自定义 memory adapter 应先在
你的应用中测试，再接入 A3S Code 流程。
