---
title: 自定义后端
description: 实现 MemoryStore 接入任意存储系统 — SQLite、Redis、向量数据库
---
...
实现 `MemoryStore` trait 即可接入任意存储系统。各后端之间真正有差异的只有 `search()` 方法——`FileMemoryStore` 使用子串匹配，向量存储则使用语义相似度。
...
store.store(MemoryItem::new("一条事实")).await?;
let results = store.search("事实", 5).await?;
```

<Callout type="info">
`MemoryStore` 是对象安全的 trait — `Arc<dyn MemoryStore>` 无需任何额外约束即可使用。
</Callout>
