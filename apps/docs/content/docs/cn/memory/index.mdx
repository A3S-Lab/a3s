---
title: A3S Memory
description: 可插拔的 AI 智能体记忆存储 — MemoryStore trait、FileMemoryStore、InMemoryStore 和相关性评分
---

import { Callout } from 'fumadocs-ui/components/callout';
import { TypeTable } from 'fumadocs-ui/components/type-table';

# A3S Memory

`a3s-memory` 是一个独立的 Rust crate，为智能体记忆提供存储层。它不感知智能体、会话或上下文注入——只负责一件事：存储和检索 `MemoryItem`。

```
核心（稳定）                    扩展（可替换）
─────────────────────────       ─────────────────────────
MemoryStore (trait)             InMemoryStore
MemoryItem                      FileMemoryStore
MemoryType                      SqliteMemoryStore（feature-gated）
RelevanceConfig                 （自定义后端）
PrunePolicy
```

## 安装

```toml
[dependencies]
a3s-memory = "0.1.1"
```

SQLite storage 需要启用 feature：

```toml
[dependencies]
a3s-memory = { version = "0.1.1", features = ["sqlite"] }
```

A3S Code v2.1.0 Node SDK 当前经过集成测试的路径，是在 `agent.session()`
options 中传入 `FileMemoryStore`：

```ts
import { Agent, FileMemoryStore } from '@a3s-lab/code';

const agent = await Agent.create('agent.acl');
const session = agent.session(process.cwd(), {
  memoryStore: new FileMemoryStore('./memory'),
});
```

## 本 crate 包含的内容

<TypeTable
  type={{
    "MemoryStore": {
      description: "异步 trait——实现或依赖的唯一接口",
    },
    "MemoryItem": {
      description: "记忆单元——内容、重要性、标签、元数据、时间戳",
    },
    "MemoryType": {
      description: "Episodic（情节）、Semantic（语义）、Procedural（程序）、Working（工作）",
    },
    "RelevanceConfig": {
      description: "评分参数——衰减、重要性权重、时效性权重",
    },
    "PrunePolicy": {
      description: "自动 pruning 参数——最大年龄、保留重要性阈值、硬性 item 上限",
    },
    "InMemoryStore": {
      description: "基于 tokio::sync::RwLock 的临时后端",
    },
    "FileMemoryStore": {
      description: "持久化后端——原子写入、内存索引、索引重建",
    },
    "SqliteMemoryStore": {
      description: "Feature-gated 持久化后端——SQLite WAL、FTS5 BM25 search、Markdown export、session log",
    },
  }}
/>

## 本 crate 不包含的内容

<TypeTable
  type={{
    "三层会话记忆（工作 / 短期 / 长期）": {
      description: "a3s-code",
    },
    "MemoryConfig（max_short_term、max_working）": {
      description: "a3s-code",
    },
    "将记忆注入智能体提示词": {
      description: "a3s-code",
    },
  }}
/>

<Callout type="info">
如果你在使用 A3S Code 构建智能体，请参阅 [Memory](/cn/docs/code/memory) 与 [API Contract](/cn/docs/code/api-contract) 了解经过集成测试的智能体层行为。
</Callout>
