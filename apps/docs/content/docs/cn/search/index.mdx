...
- **ACL 配置** — 从 `.acl` 配置文件加载引擎权重、健康设置和超时
- **无头浏览器** — 通过 `headless` 启用 Chrome/Chromium，也可通过 `lightpanda` 启用 Lightpanda，用于 JS 渲染引擎（Google、百度、必应中国）
- **自动安装浏览器** — 未找到浏览器时自动检测或下载 Chrome for Testing；启用 `lightpanda` 特性时可检测或下载 Lightpanda
...
- **跨平台** — macOS（arm64、x64）、Linux（x64、arm64）和 Windows（x64），支持自动下载浏览器
...
<TypeTable
  type={{
    "Rust": {
      description: "包: [a3s-search](https://crates.io/crates/a3s-search) · 安装: Cargo.toml 中添加 a3s-search = '1.3'",
    },
    "Python": {
