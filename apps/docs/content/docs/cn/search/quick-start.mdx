...
Rust 库默认只启用 HTTP 引擎。无头浏览器引擎（Google、百度、必应中国）需要手动启用 `headless`：

```toml
a3s-search = "1.3"  # 仅 6 个 HTTP 引擎
```

启用 Chrome/Chromium 无头浏览器支持：

```toml
a3s-search = { version = "1.3", features = ["headless"] }  # 9 个引擎
```

在 Linux/macOS 上启用 Lightpanda 支持时使用 `lightpanda`；它会隐式启用 `headless`：
