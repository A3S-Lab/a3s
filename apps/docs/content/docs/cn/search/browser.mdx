...
- **Chrome/Chromium** — 通过 `headless` 特性启用；可自动检测本机浏览器，也可下载 Chrome for Testing
- **Lightpanda** — 通过 `lightpanda` 特性在 Linux/macOS 上启用；该特性会隐式启用 `headless`

`BrowserBackend::default()` 在编译启用 `lightpanda` 时选择 Lightpanda，否则选择 Chrome。

## 功能开关

无头浏览器支持在 Rust crate 中需要手动启用：

```toml
# Rust 库（默认只启用 HTTP 引擎）
[dependencies]
a3s-search = "1.3"

# 使用 Chrome/Chromium 无头后端
[dependencies]
a3s-search = { version = "1.3", features = ["headless"] }

# 使用 Lightpanda 后端（仅 Linux/macOS；隐式启用 headless）
[dependencies]
a3s-search = { version = "1.3", features = ["lightpanda"] }
```

Rust crate 当前只暴露两个相关特性：`headless` 和 `lightpanda`。
...
编译启用 `features = ["lightpanda"]` 后，也可以使用 `BrowserBackend::Lightpanda` 和 `lightpanda_path`。

### 配置项

<TypeTable
  type={{
    backend: { type: 'BrowserBackend', default: 'Chrome，启用 lightpanda 特性时为 Lightpanda', description: '由编译期特性选择的浏览器后端' },
...
当 `chrome_path` 或 `lightpanda_path` 为 `None` 时，A3S Search 会自动检测或下载浏览器：

### Lightpanda（启用 `lightpanda` 特性时）

1. `LIGHTPANDA` 环境变量
2. 系统 PATH（`lightpanda` 命令）
3. 缓存目录：`~/.a3s/lightpanda/<tag>/lightpanda`
4. 从 GitHub releases 自动下载

支持的平台：macOS（arm64、x64）和 Linux（x64、arm64）。首次运行可能需要下载约 59MB。
...
4. 缓存目录：`~/.a3s/chromium/<version>/`
5. 从 Google CDN 自动下载 Chrome for Testing
...
<TypeTable
  type={{
    ...
    "BrowserFetcher": {
      description: "通过 BrowserPool 进行无头浏览器渲染",
    },
  }}
/>
