...
<TypeTable
  type={
    {`\`a3s-box-core\``}: {
      description: "共享类型：配置、错误、事件、网络模式、卷/PTY/DNS 类型",
    },
    {`\`a3s-box-runtime\``}: {
      description: "VM 生命周期、OCI 镜像、热池、TEE、网络、gRPC 客户端",
    },
    {`\`a3s-box-cli\``}: {
      description: "类 Docker CLI，52 个命令，状态持久化，输出格式化",
    },
    {`\`a3s-box-cri\``}: {
      description: "Kubernetes 容器运行时接口（CRI）gRPC 实现",
    },
    {`\`a3s-box-shim\``}: {
      description: "VM 子进程，通过 libkrun（HVF/KVM）桥接宿主机和 VM",
    },
    {`\`a3s-box-guest-init\``}: {
      description: "Guest PID 1：挂载文件系统、启动服务、运行用户入口",
    },
    {`\`a3s-box-sdk\``}: {
      description: "嵌入式沙箱 SDK：从 Rust 代码创建、执行、停止 MicroVM",
    },
  }
/>
...
<TypeTable
  type={
    {`\`Created\``}: {
      description: "配置已捕获，VM 尚未启动",
    },
    {`\`Ready\``}: {
      description: "VM 已启动，Agent 已初始化，gRPC 健康",
    },
    {`\`Busy\``}: {
      description: "会话正在活跃处理中",
    },
    {`\`Compacting\``}: {
      description: "上下文压缩进行中",
    },
    {`\`Stopped\``}: {
      description: "VM 已终止，资源已释放",
    },
  }
/>
...
<TypeTable
  type={
    "4088": {
      description: "服务: Agent 控制 · 协议: gRPC（健康检查、指标）",
    },
    "4089": {
      description: "服务: Exec 服务 · 协议: gRPC（命令执行）",
    },
    "4090": {
      description: "服务: PTY 服务 · 协议: gRPC（终端 I/O）",
    },
    "4091": {
      description: "服务: Attestation 服务 · 协议: gRPC + RA-TLS（TEE 操作）",
    },
  }
/>
