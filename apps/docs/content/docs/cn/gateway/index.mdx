...
| 组件 | 职责 |
|------|------|
| Gateway | 顶层编排器 — 将配置、监听器、路由、扩缩容和可观测性整合为一个单元 |
| Entrypoint | 带可选 TLS 的命名端口监听器（HTTP、HTTPS、TCP、UDP） |
| Router | 按 Host、Path、PathPrefix、Headers、Method 和 SNI 匹配请求 |
| Middleware | 可组合的请求/响应转换（15 个内置中间件） |
| Service | 带负载均衡、健康检查、故障转移和流量镜像的后端池 |
| Provider | 动态配置来源（文件监听热重载、DNS 发现、Kubernetes） |
| Proxy | 协议专用请求转发（HTTP、WebSocket、gRPC、SSE、TCP、UDP） |
| Scaling | Knative 风格自动扩缩容，支持并发限制、请求缓冲、版本路由和灰度发布 |

## 协议支持

| 协议 | 特性 |
|------|------|
| HTTP/1.1 & HTTP/2 | 完整反向代理，过滤逐跳头 |
| WebSocket | 升级检测、双向中继、多路复用 |
| SSE / 流式 | 分块传输，适用于 LLM 输出和事件流 |
| gRPC | HTTP/2 h2c 转发，头部转换 |
| TCP | 原始字节中继，SNI 路由，IP 过滤 |
| UDP | 基于会话的数据报中继 |
