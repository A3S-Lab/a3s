...
A3S Gateway 是 AI 原生服务的流量层。基于 [hyper](https://hyper.rs/) 和 [rustls](https://github.com/rustls/rustls) 构建，以单一静态链接二进制发布，配置完全使用 ACL（Agent Configuration Language）定义。
...
热重载通过共享运行时快照原子替换路由表和服务注册表。HTTP/TCP entrypoint 配置不变时不会重新绑定流量端口；新增或迁移 HTTP/TCP listener 时按名称增量协调，如果新端口绑定失败，未变化的 listener 会继续工作。UDP entrypoint 仍会显式重启。
...
<TypeTable
  type={{
    "HTTP/1.1 & HTTP/2": {
      description: "完整反向代理，含逐跳头过滤、标准化 X-Forwarded-* 元数据和连接池管理",
    },
    "SSE / 流式传输": {
...
- 转发头 — 上游 HTTP 请求自动带有标准化的 `X-Forwarded-For`、`X-Forwarded-Host`、`X-Forwarded-Proto` 和 `X-Forwarded-Port`
- 纯 Rust TLS（rustls）— ACME/Let's Encrypt，支持 HTTP-01 和 DNS-01（Cloudflare、Route53）
- 热重载 — inotify/kqueue 文件监听，HTTP/TCP entrypoint 不变时无需重绑端口
...
- 仅 ACL 配置 — 一致、可读、可版本控制
- CLI-first 管理 — 可选的认证/mTLS Dashboard API 运行在独立 listener 上
