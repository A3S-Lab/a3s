...
`request_timeout` 控制 Gateway 等待普通缓冲 HTTP 上游响应的最长时间，超时后返回
`504 Gateway Timeout`。支持毫秒、秒或分钟，例如 `"500ms"`、`"30s"` 或 `"2m"`。

<TypeTable
  type={{
    strategy: { type: 'string', default: '"round-robin"', description: '负载均衡策略' },
    request_timeout: { type: 'string', default: '"30s"', description: '普通缓冲 HTTP 上游请求超时，超时后返回 504' },
    servers: { type: 'array', required: true, description: '后端 URL 和可选权重' },
  }}
/>
