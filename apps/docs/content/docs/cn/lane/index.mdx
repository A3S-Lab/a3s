...
|------|--------|----------|------|
| `system` | 0（最高） | 5 | 系统级操作 |
| `control` | 1 | 3 | 暂停、恢复、取消 |
| `query` | 2 | 10 | 只读查询 |
| `session` | 3 | 5 | 会话管理 |
| `skill` | 4 | 3 | 技能/工具执行 |
| `prompt` | 5（最低） | 2 | LLM 提示处理 |
...
| 语言 | 包 | 安装 |
|------|-----|------|
| Rust | [`a3s-lane`](https://crates.io/crates/a3s-lane) | Cargo.toml 中添加 `a3s-lane = "0.4"` |
...
| 场景 | 吞吐量 |
|------|--------|
| 并发（10 通道） | 33,000–50,000 ops/sec |
| 单通道 | 6,600–10,000 ops/sec |
| 完整生命周期（创建/启动/关闭开销） | ~85–93 ops/sec |
| 指标开销 | ~3–5% |
