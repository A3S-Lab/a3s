...
- **TEE 可审计推理**：默认使用纯 Rust `mistralrs` 保持兼容性，并提供面向层流式 TEE 部署的 `picolm` / `tee-minimal`
...
<TypeTable
  type={{
    ...
    "推理后端层": {
      description: "可插拔推理引擎：mistralrs（纯 Rust，默认）、llama.cpp（可选）、picolm（TEE 层流式推理，可选），以及面向 Remote manifest 的 proxy 上游。",
    },
    "模型层": {
...
<TypeTable
  type={{
    "GGUF": {
      description: "量化模型（Q4_K_M、Q8_0 等）。根据 feature 和路由，可由 mistralrs、llama.cpp 或 picolm 加载。",
    },
    "SafeTensors": {
    ...
    "Remote": {
      description: "配置 proxy_upstreams 后，通过 proxy 后端服务 OpenAI 兼容上游。",
    },
  }}
/>
