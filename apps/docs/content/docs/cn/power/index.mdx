...
| 组件 | 说明 |
|------|------|
| `cli` | 12 个命令：pull、run、list、show、delete、serve、ps、stop、push、cp、create |
| `api` | 14 个 Ollama 原生端点 + 5 个 OpenAI 兼容端点 |
| `backend` | 可插拔推理引擎，内置 llama.cpp 实现 |
| `model` | 内容寻址存储、注册表、解析、GGUF 读取器 |
| `server` | Axum HTTP 服务器，支持 CORS、追踪、Prometheus 指标 |
| `config` | TOML 配置，兼容 Ollama 环境变量 |
...
| 路径 | 说明 |
|------|------|
| `~/.a3s/power/` | 基础目录（可通过 `$A3S_POWER_HOME` 覆盖） |
| `~/.a3s/power/config.toml` | 配置文件 |
| `~/.a3s/power/models/manifests/` | 模型 manifest 文件 |
| `~/.a3s/power/models/blobs/` | 内容寻址 blob 存储 |
