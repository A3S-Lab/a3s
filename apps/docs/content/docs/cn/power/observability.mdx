...
<TypeTable
  type={{
    ...
    "power_active_requests": { description: "当前活跃推理请求数" },
    "power_requests_running": { description: "已经获得 admission permit、正在运行的推理请求数" },
    "power_requests_waiting": { description: "达到 max_concurrent_requests 后，正在等待 admission permit 的排队请求数" },
  }}
/>

`max_concurrent_requests` 使用背压而不是直接拒绝超量推理请求。等待中的请求计入 `power_requests_waiting`，已放行请求会持有 permit 直到流式响应结束。
...
<TypeTable
  type={{
    "power_gpu_utilization": { description: "GPU 计算利用率，范围为 0.0-1.0" },
    "power_gpu_memory_bytes": { description: "按 device 标签统计的 GPU 内存占用字节数" },
  }}
/>
