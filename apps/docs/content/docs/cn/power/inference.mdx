...
A3S Power 使用可插拔的 `Backend` trait，提供三种实现：
...
<TypeTable
  type={{
    ...
    "picolm（可选）": {
      description: "纯 Rust 层流式 GGUF 推理。峰值内存 = O(layer_size) 而非 O(model_size)。可在 512MB TEE EPC 中运行 7B+ 模型。Feature 标志：--features picolm。",
    },
  }}
/>
...
**`picolm.rs` — 层流式前向传播**：依次遍历每个 transformer 层（`blk.0.*` 到 `blk.{n-1}.*`），将权重应用到隐藏状态，然后继续下一层。处理完第 N 层后，其权重页面不再被引用，操作系统可以在第 N+1 层换入之前回收它们。
...
对于加密模型（`.enc`），`LayerStreamingDecryptedModel` 逐块解密。每个块包装在 `Zeroizing<Vec<u8>>` 中——丢弃时自动清零。任何时刻内存中只存在一层的明文权重。

### 当前状态

层流式基础设施（mmap 解析器、逐层迭代、采样、加密流式传输）已达到生产就绪状态。前向传播目前使用占位算术——真正的 transformer 矩阵运算是下一个里程碑。
