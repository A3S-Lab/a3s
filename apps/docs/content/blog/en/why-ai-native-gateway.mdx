...
A3S Gateway's design trade-off is: only do what an AI service traffic layer needs, fully described in an ACL config file, deployed as a single binary. No database, no control plane, no Kubernetes required (though supported).
...
middlewares "jwt-auth" {
  type  = "jwt"
  value = env("JWT_SECRET")            # read secret from environment variable
}
...
COPY gateway.acl /etc/a3s-gateway/gateway.acl

USER gateway

EXPOSE 8080 443

ENTRYPOINT ["a3s-gateway", "--config", "/etc/a3s-gateway/gateway.acl"]
```
...
[A3S Box](https://github.com/AI45Lab/Box) is a microVM-based sandbox runtime. In scenarios where a full Kubernetes cluster is not needed — such as edge nodes, development machines, or resource-constrained single servers — Box can replace Docker Compose to manage the lifecycle of the gateway and LLM instances.

Box configuration is also ACL. Create `box.acl`:

```text
# box.acl — run gateway + Ollama in microVM sandboxes

workloads "gateway" {
  binary = "/usr/local/bin/a3s-gateway"
  args   = ["--config", "/etc/gateway/gateway.acl"]
...
Prepare `values-prod.yaml` with the full ACL config embedded:
