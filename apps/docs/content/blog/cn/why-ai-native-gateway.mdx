...
A3S Gateway 的设计取舍是：只做 AI 服务流量层需要的事，用 ACL 配置文件完整描述，单个二进制部署。不需要数据库、不需要控制平面、不需要 Kubernetes（尽管支持）。
...
middlewares "jwt-auth" {
  type  = "jwt"
  value = env("JWT_SECRET")            # 从环境变量读取密钥
}
...
[A3S Box](https://github.com/AI45Lab/Box) 是一个基于 microVM 的沙箱运行时。在不需要完整 Kubernetes 的场景下——比如边缘节点、开发机、或者资源受限的单台服务器——可以用 Box 替代 Docker Compose 来管理网关和 LLM 实例的生命周期。

Box 的配置也是 ACL。创建 `box.acl`：

```acl
# box.acl — 在 microVM 沙箱中运行网关 + Ollama
...
先准备 `values-prod.yaml`，把完整的 ACL 配置嵌进去：
