...
|---|---|---|---|
| **核心定位** | 开发者终端工具 | 开放生态终端工具 | 可嵌入编码 Agent 库 |
| **使用方式** | CLI 交互 | CLI 交互 | 代码调用 |
| **模型支持** | Anthropic（+ Bedrock/Vertex） | 任意 OpenAI 兼容 | 任意 OpenAI 兼容 |
| **可嵌入性** | 无编程 API | 无编程 API | 核心设计目标 |
| **扩展方式** | MCP + Skills + Hooks | MCP + 插件 | Trait 扩展点 + MCP |
| **多租户** | 不支持 | 不支持 | 支持 |
| **并行任务** | 子 Agent 并行 | 有限支持 | Lane 队列 + 多机器分发 |
| **开源** | 否 | 是 | 是 |

这不是功能优劣的对比，是设计目标的对比。三个工具在各自的定位上都做得很好。
