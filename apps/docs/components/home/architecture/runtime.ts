...
export const runtimeArchitectureProjects: readonly ArchitectureProject[] = [
  ...
  },
  {
    ...
    name: 'Flow',
    category: 'runtime',
    role: localized('为 Agent、工具、审批和子工作流提供可恢复的工作流引擎。', 'Provides a recoverable workflow engine for Agents, tools, approvals, and child workflows.'),
    href: getProjectPrimaryHref('flow'),
    nodes: [
      node('engine', 'FlowEngine', 'core', '从事件历史恢复运行，再返回下一项持久决定。', 'Restores a run from event history and returns the next durable decision.'),
      ...
      node('authoring', 'React / Vue', 'surface', '18 个节点 manifest 共用卡片、配置面板和 Hook。', 'Shares 18 node manifests across cards, configuration panels, and hooks.'),
      node('automation', 'CLI / Skill', 'contract', '查询节点字段，创建节点，校验并编译作者图。', 'Queries node fields, creates nodes, validates graphs, and compiles authoring documents.'),
