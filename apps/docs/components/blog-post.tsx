...
import { createLocalizedMdxLink } from '@/lib/localized-mdx-link';
import {
  localePath,
  localizedUrl,
  openGraphLocale,
...
export function BlogPost({ locale, slug }: BlogPostProps) {
  ...
  return (
    ...
    >
      <SiteNav lang={locale} section={isChinese ? '博客' : 'Blog'} />
...
export function blogPostMetadata({ locale, slug }: BlogPostProps): Metadata {
  const post = getBlogPost(locale, slug);
  if (!post) notFound();
