...
const docsRoot = path.join(root, "apps/docs/docs");
const release = JSON.parse(
  readFileSync(
    path.join(root, "apps/docs/cloud-releases/cloud-0.1.0.json"),
    "utf8",
  ),
);
const pages = [
...
function parseBlocks(source, kind) {
  ...
  for (const match of source.matchAll(expression)) {
    ...
    for (const line of match[2].split("\n")) {
      ...
      if (attribute)
        attributes.set(attribute[1], attribute[2] ?? Number(attribute[3]));
    }
...
const lockSource = readFileSync(
  path.join(root, release.compatibilityLock),
  "utf8",
);
...
assert.equal(
  gitlink,
  release.cloudRevision,
  "Cloud gitlink differs from the docs snapshot",
);

function resolveLocalLink(language, pagePath, target) {
  const localized = target.match(/^\/(?:(cn|zh|en)\/)?docs\/(.+)$/);
  if (localized) {
    const requestedLanguage = localized[1] ?? language;
    const targetLanguage =
      requestedLanguage === "cn" ? "zh" : requestedLanguage;
    return path.join(docsRoot, targetLanguage, "docs", localized[2]);
  }
...
function navigationNames(meta) {
  return meta.map((item) => (typeof item === "string" ? item : item.name));
}

for (const language of ["en", "zh"]) {
  const cloudDirectory = path.join(docsRoot, language, "docs/cloud");
  const cloudMeta = JSON.parse(
    readFileSync(path.join(cloudDirectory, "_meta.json"), "utf8"),
  );
  assert.deepEqual(
    navigationNames(cloudMeta),
    ["index", "v0.1.0"],
    `${language} Cloud root navigation drift`,
  );
  const versionDirectory = path.join(cloudDirectory, "v0.1.0");
  const versionMeta = JSON.parse(
    readFileSync(path.join(versionDirectory, "_meta.json"), "utf8"),
  );
  assert.deepEqual(
    navigationNames(versionMeta),
    pages,
    `${language} Cloud topic parity drift`,
  );
  ...
  for (const page of pages) {
    ...
    assert.ok(
      source.includes("Snapshot: `cloud-0.1.0`"),
      `${pagePath} has no snapshot marker`,
    );
    assert.match(
      source,
      /Status: \*\*(Verified|Experimental|Planned|Optional)\*\*/,
    );
    assert.ok(
      !source.includes("a3s cloud "),
      `${pagePath} advertises the planned Cloud CLI`,
    );
    ...
    for (const fence of source.matchAll(/```(bash|sh)\n([\s\S]*?)```/g)) {
      ...
      if (process.platform !== "win32") {
        ...
        assert.equal(
          checked.status,
          0,
          `${pagePath} has invalid shell syntax: ${checked.stderr}`,
        );
      }
    }
  }
...
assert.deepEqual(
  release.commandHelp,
  [],
  "planned command surfaces must not publish help fixtures",
);
