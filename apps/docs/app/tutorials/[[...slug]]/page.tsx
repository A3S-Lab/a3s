    ...
    <main className="min-h-screen" style={{ background: 'var(--ct-bg)', color: 'var(--ct-text)', fontFamily: 'var(--ct-font)' }}>
      ...
      <footer className="border-t border-slate-200 px-4 py-8 dark:border-slate-700/60 sm:px-6">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          ...
          <Link href="/" className="text-sm text-slate-400 transition-colors hover:text-indigo-600 dark:text-slate-500 dark:hover:text-indigo-400">← Home</Link>
        </div>
      </footer>
    </main>
  );
...
export default async function Page({ params }: PageProps) {
  ...
  // No slug = list page
  if (!slug || slug.length === 0) return <TutorialsList />;

  const page = tutorialsSource.getPage(slug, 'en');
  if (!page) notFound();
    ...
    <main className="min-h-screen" style={{ background: 'var(--ct-bg)', color: 'var(--ct-text)', fontFamily: 'var(--ct-font)' }}>
      ...
      <div className="border-b border-slate-200 dark:border-slate-700/60 px-4 py-3 sm:px-6">
        <div className="mx-auto max-w-7xl">
          <Link href="/tutorials" className="inline-flex items-center gap-1.5 text-sm text-slate-400 transition-colors hover:text-indigo-600 dark:text-slate-500 dark:hover:text-indigo-400">
...
export function generateStaticParams() {
  return [
    { slug: undefined },
    ...tutorialsSource.getPages('en').map((page) => ({
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  if (!slug || slug.length === 0) return { title: 'Tutorials', description: 'Step-by-step guides for building with A3S.' };
  const page = tutorialsSource.getPage(slug, 'en');
  if (!page) notFound();
